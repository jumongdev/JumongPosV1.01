...
## Latest Change (2026-09-25) — ✅ FINANCE CHECK sa End Shift: mark shifts as checked by finance (check/uncheck, sino + kailan); API 1.1.85 + 🚨 SERVER ACCESS FIX (ADMIN/Jum0ng!Server26, name-based, IP .20)

**Request:** "mark the endshift as checked by finance" — per-row ✓ CHECK button + uncheck sa dashboard, recorded sino at kailan; pareho sa POS client shifts at mobile POS shifts; dashboard lang muna (walang POS client change). **DB:** `daily_closes` + `wh_daily_closes` → `checked_by TEXT DEFAULT ''` + `checked_at TIMESTAMPTZ` (migrations sa PgDatabaseHelper). **API (1.1.85):** `POST /dashboard/shift-check` (daily_closes by storeId+posId) + `POST /dashboard/warehouse/shift-check` (wh_daily_closes by id) — body `{checked, checkedBy}` (uncheck = ''/NULL + cleared checked_at; `@checked` verbatim identifier — C# reserved word); `GET /shift-history` + `GET /warehouse/shifts` → + `posId` (shift-history) / `checkedBy`/`checkedAt`. **Dashboard (cache-buster v=20260924a):** Shift History + Mobile POS END SHIFT tables → **FINANCE column** — ✅ badge (checkedBy · date, click = uncheck) o **✓ MARK CHECKED** button; `checkShift(x, checked)` sa `shiftHistory` + `mobilePos` components (checkedBy = `jpos_web_user`); CSV exports + Checked By/Checked At. **Verified live (2026-09-25):** mark/unmark round-trip sa parehong endpoints ✓ (TEST-FINANCE, na-revert); warehouse/shifts + shift-history may checked fields ✓; version 1.1.85 live ✓. **🚨 SERVER ACCESS GOTCHA (na-resolve):** ang server ay nasa **192.168.1.20** na (DHCP change) at ang gumaganang account ay **`ADMIN` / `Jum0ng!Server26`** — HINDI `remotedev`/`Jum0ng!Dev55` (old docs, HQ lang iyon) — at **connect by MACHINE NAME lang** (IP → 0x8009030e). Na-verify via SMB IPC$ + WinRM; na-update na sa AGENTS.md (WinRM table, Machine Roles, deploy snippets). Deploy: stop → copy publish → start → version 1.1.85 ✓. Git commit `36df267` (feature) + docs commit. Walang POS client release.

## Latest Change (2026-09-21) — 🕊 DOVE master catalog fix: naibalik ang "nawawalang" stock sa Product Management (reactivate 161 + bagong master 758)

**Report:** "why zero when the store still has?" (DOVE items sa PRODUCT MANAGEMENT). **Root cause:** (1) ang **DOVE CONDITIONER SERUM HAIR FALL RESCUE (GREEN) 10ML** (barcode `4800888206572`, HQ stock **44**) ay master **id 161** pero **INACTIVE** — hindi lumalabas sa default na "Active" filter; (2) ang **DOVE CONDITIONER STRAIGHT AND SILKY (PINK) 12ML** (barcode `4800888283214`, HQ stock **41**) ay **WALANG master product** — kaya wala sa panel kahit anong filter. Ang mga active DOVE master (162-167) ay talagang **0** sa lahat ng stores (maliban sa GOLD=2 sa HQ) — tama ang 0. **Fix (data lang, via dashboard API):** (1) **reactivate master 161** (`PUT /products/master/161`, isActive=true); (2) **create master id 758** para sa PINK 12ML (barcode `4800888283214`, category conditioner, price ₱42, cost ₱41 — mula sa HQ local row 5236; sellOnline=true). **Verified (headless Chrome):** Product Management → DOVE GREEN **total=44, hq=44** ✓; DOVE PINK 12ML **total=41, hq=41** ✓; iba pang DOVE = 0 (tama). **Bonus audit (hindi pa ginalaw — DOVE lang ang request):** **17 inactive master** na may store stock (~440 pcs: 555 SARDINES 101, PUREFOODS CORNED BEEF 150G 46, DEL MONTE TOMATO SAUCE 44, TATTOOS CHEDDAR 33, RENO LIVER 32, Absolute 8L 24, RSL Crackers 46, MALING CLM 12, Hasmin rice 14, PUREFOODS LUNCHEON 3, ROSE BOWL 1) + 1 lang ang walang master (DOVE PINK — naayos na).

## Latest Change (2026-09-21) — 🚚 STORE TRANSFERS panel: nakita na ang store-to-store (mobile) transfers — Source filter + From column

**Report:** "i saw wh transfer 1181 but not in the dashboard transfer report". **Root cause:** ang dashboard Inventory → Store Transfer panel ay may **hardcoded `source=hq`** (`components.js` `storeTransferPanel.load()`) — kaya ang **store-to-store transfers na gawa ng mobile app** (`source='store'`, hal. **#1180/#1181/#1186 U Got → HQ/ACGS**, `created_via=mobile`) ay HINDI lumalabas. DB: 323 hq + **6 store** + 767 legacy warehouse. **Fix (wwwroot only, walang API change):** (1) **Source dropdown** — ALL SOURCES (default) / HQ → STORE / STORE → STORE; (2) bagong **From column** (origin: `from_store_id` → store name, o `HQ`/`🏭 WH`); (3) title → **"STORE TRANSFERS"** + dynamic na source hint; (4) CSV export + From/Source columns. **Verified (headless Chrome):** default total=**1096** at **lumalabas na ang #1181** (ACGS - Naic Market, from U Got, via mobile) ✓; filter STORE = **6 rows** (lahat galing U Got) ✓; filter HQ = **323** ✓. Cache-buster `v=20260921c` → **`v=20260921d`**.

## Latest Change (2026-09-21) — 🛡 POINTS SAFETY follow-up (A+B): cloud email/phone fallback match + POS `updated=0` guard; **POS v1.1.84 released (GitHub)**

**Request:** "recommended" (A+B) — para hindi na mawala ang points sa offline/name-mismatch na sitwasyon. **(A) Cloud (API 1.1.84):** `SyncCustomers` — kapag 0 rows ang name match, susubukan ang **EMAIL** tapos **PHONE** match (google-registered row lang, `ORDER BY id LIMIT 1` — isang row lang, hindi ma-contaminate ang ibang account). Verified live: maling name + tamang email → `updated:1`; maling name + walang email + tamang phone → `updated:1`; walang match → `updated:0, skipped:1`; hindi nagbago ang pangalan/points ni allaija ✓. **(B) POS client (v1.1.84):** bagong `SyncService.SyncCustomer` — hindi na `PostAsync` (HTTP 200 lang) ang basehan; binabasa na ang response body at kapag **`updated<=0`** ay **hindi naka-clear ang `PointsDirty`** (nananatiling pending locally — retry sa susunod na award/void o manual SYNC ALL; protektado rin ng PointsDirty ang local points sa 5-min customer pull). **(Offline flow na-verify sa code):** customer picker/QR scan = local SQLite; points = local award + `SyncQueue` (30s retry) + `PushAllUnsyncedAsync` (30s) — kaya offline OK at naipapasok ang points pagbalik online (maliban sa bagong customer na hindi pa na-download). **(Release):** client publish → `C:\dev\out\client` → WinRM push sa `C:\JumongAPI\client\` + **GitHub release `v1.1.84`** (asset `JumongPosV1.01.exe`, 211,707,324 bytes, `releases/latest` = v1.1.84 ✓, download URL 302→200 ✓). AppVersion **1.1.84** + dashboard `latestVer` **1.1.84** (mag-OUTDATED ang stores hanggang mag-UPDATE APP). **Stores: i-tap ang UPDATE APP** (Settings) — walang APK change.

## Latest Change (2026-09-21) — 🚨 NAWAWALANG ACCOUNTS FIX: customers list LIMIT 500 → WALANG LIMIT (33 active R-Z hidden); API 1.1.84

**Report:** "pati mga account nag kanda walaan" + "limit pati sa search ayaw". **Root cause:** ang dashboard `/dashboard/customers` ay may hardcoded **`LIMIT 500`** pero **581 customers** na ang nasa cloud — kaya **33 ACTIVE customers** (pangalan R–Z: Rocelyn Rabino, Roda timoteo, Rojean Mahumas, Rolyn Buenaventura, atbp.) ay **hindi lumalabas** sa Customers panel AT **hindi mahahanap ng search** (500 rows lang ang naka-load sa client). **Fix (API 1.1.84):** `limit=0` (bagong default) = **WALANG LIMIT** — lahat ng customers; optional pa rin ang `limit` param (max 100000) + bagong **server-side `search` param** (name/phone/email/QR ILIKE) para sa susunod na gamit. **Verified:** `/customers` = **581 rows** (dati 500); dashboard search — `Rocelyn` → Rocelyn Rabino (active, 14 pts), `Roda` → Roda timoteo (active, 30 pts), `romelyn` → Romelyn Permejo (12 pts), `allaija` → 10 pts ✓; **hindi kasama ang inactive** ✓; active count = 238. Hindi naapektuhan: whmobile picker (`?search=&qrOnly=true`) at POS customer pull (`?all=true`, walang limit).

## Latest Change (2026-09-21) — 🔍 CUSTOMERS search box (name/phone/email/QR; hindi kasama ang INACTIVE); dashboard cache-buster v=20260921c

**Request:** "can you put search in customer so i can search by name" + "dont include the inactive in search". **Change (wwwroot only, walang API build):** bagong search input sa Customers panel header (`components.js` `customersList` + `index.html`) — `x-model="search"` na may ✕ clear button at live na **"N resulta"** counter. Kapag may query, hinahanap sa **LAHAT ng ACTIVE customers** (name/phone/email/QR, case-insensitive) — hindi na pinipigilan ng ⭐/⛔ filter para siguradong makita ang customer, pero **HINDI kasama ang INACTIVE** (manual/lumang accounts, `isActive === false`); kapag walang query, bumalik sa dating filter behavior (may "may inactive" toggle pa rin). Empty state: "Walang customer na tumutugma sa search." **Verified (headless Chrome + CDP):** total=500 (295 inactive); `allaija` → 1 result (pts=10 ✓); `abad` → **butch abad 2 (4 pts) + MARICRIS ABAD (10 pts) lang — WALA ang inactive na "abad store"** ✓; `a` → 205 results, **lahat active** ✓; `abigail` → active "Abigail Marie Baleña" lang (wala ang inactive "abigail") ✓; `ALLAIJA`/phone `9054608732`/QR `AS-A49F04` → allaija ✓; clear → balik sa 205. `node --check components.js` OK.

## Latest Change (2026-09-21) — 🚨 CRITICAL POINTS BUGFIX: na-wipe ang loyalty points ng QR customers (SyncCustomers camelCase read) + 52-customer repair; API 1.1.83

**Report:** "allaija deocareza ... 0 ⭐EARN ... no point earn daw" + "since 17 daw wala points sila" + "dedemanda nila ako". **Root cause:** ang v1.1.82 na `SyncCustomers`/`SyncUsers` (bagong update-only code, na-deploy ~Sep 16-17) ay **snake_case lang** ang binabasa (`pos_id`, `loyalty_points`, `is_active`, `credit_balance`...) pero ang POS ay **camelCase** ang JSON (PostAsync naming policy → `posId`, `loyaltyPoints`, `isActive`...) → `TryGetProperty` fail → **pos_id=0 at loyalty_points=0 sa BAWAT push**. Kaya: (1) bawat QR sale push ay nag-set ng points=0 sa cloud (kahit may 2/8 points ang sale header); (2) `SyncLog` = "OK" (HTTP 200) kaya walang error badge kahit walang na-update; (3) ang POS ay nag-clear ng `PointsDirty` → ang 5-min customer download ay nag-revert din ng LOCAL points sa 0. **Fix (API 1.1.83):** bagong `TryGetProp()` dual lookup (snake_case + camelCase, gaya ng `SyncTable`) sa `SyncCustomers` + `SyncUsers`. **Verified live:** camelCase push test → `{"ok":true,"updated":1}` (dati nagma-match pero 0-values ang isinusulat), allaija pos_id=4088 + points=10 ✓. **Data repair (cloud PG, 52 customers, +300 pts):** `loyalty_points = GREATEST(current, SUM(sales.total_points_earned matched by customer_qr) + ecom delivered points)` — allaija 0→**10** (2+8 ✓), Nelia Rosales 9→26, Flordeliza Chia 0→16, gemmaleen magbanua 0→15, KrisBee Hong 0→15, Ginalyn Santiago 1→14, Merilyn Paguya 0→12... Walang duplicate QR codes (safe ang QR matching). POINTS LIABILITY ngayon: **122 customers / 1,207 points (₱1,207)**. **Bonus fix:** dashboard customer orders ay `SUM(sale_items.points_earned)` ang gamit (0 sa cloud dahil ang POS ay in-memory cart items ang ini-sync na PointsEarned=0 — hiwalay na client bug) → fallback na sa `sales.total_points_earned`. **FOLLOW-UPS (POS client, next release):** (a) `SaleService.SaveSale` (line 223) — i-re-read ang per-item PointsEarned mula sa DB bago mag-`SyncSale` (para tama ang per-item points sa cloud); (b) huwag i-clear ang `PointsDirty` kapag `updated=0` ang cloud response (name mismatch protection).

## Latest Change (2026-09-21) — 📅 Mobile POS daily view (TODAY | ALL | 📅 date picker) + 🛒 E-COMMERCE card; API 1.1.82

**Request:** "check report ng mobile pos reporting mix day — pwede ba yung transaction nakikita daily lang tapos yung previous pick by day date picker" + "sa dashboard panel pwede ba nakikita rin ang ecommerce sales mobile pos sale ng ugot at hq". **Changes:** **(1) Day-based view (SALES + END SHIFT):** tinanggal ang 7D/30D/Month presets sa dashboard Mobile POS panel (Reports); bagong **TODAY | ALL | 📅 date picker** (default TODAY; flat list, walang per-day grouping) + day label chip (TODAY / ALL / `Sep 20, 2026`) — gaya ng whmobile `setSalesDay`. Ang parehong araw ang gamit ng SALES, END SHIFT at E-COMMERCE. **(2) API 1.1.82:** `/dashboard/warehouse/shifts` may bagong optional `from`/`to` (`close_date` filter; backward-compatible — walang filter kapag wala, hindi apektado ang mobile app at Shift History panel) at bagong `GET /dashboard/ecom-sales?from=&to=&limit=500` — `online_orders` na `status='delivered'` lang, `delivered_at` basehan (tulad ng HQ Outflow), may `deliveredAt`/`driverName`/`itemCount`; `COUNT(*)/SUM() OVER ()` = **buong-range totals** kahit ma-limit ang rows. **(3) 🛒 E-COMMERCE (HQ ONLINE) card** sa SALES tab — per-day delivered orders (Order No · Delivered · Customer · Driver · Payment · Items · Total) + TOTAL footer + CSV + clickable item detail (existing `/shop/orders/{id}`; `total`→`subtotal` mapping para sa sale modal). **HQ-only:** walang `store_id` ang `online_orders` (HQ stock + HQ driver ang nagde-deliver) kaya hiwalay ang card sa U Got/HQ mobile POS cards — hindi ito maa-attribute sa U Got. **Verified (live server, headless Chrome + CDP):** version=**1.1.82** ✓; default = TODAY (`from=to=2026-09-21`, label TODAY) ✓; U Got sales today=10, Sep 20=**55** ✓; e-com Sep 20 = **28 orders / ₱68,276.75** (tugma sa raw API ✓), ALL = 307 orders; modal = SHOP-20260920-0466 / Zeny Fullido / 10 items / Redhorse 1L subtotal 650 ✓; shifts `from/to` — HQ 2 / U Got 1 (Sep 1-21) ✓; `node --check components.js` OK; cache-buster `v=20260919c` → **`v=20260921a`** (i-refresh ang browser). Walang POS client/APK change.

## Latest Change (2026-09-20) — 🕐 Transfer timestamps (created + accepted) + "Via" app + DATE FILTER FIX (500 error); API 1.1.81 · whmobile 20260919-13

**Request:** "dapat may timestamp — time created at anong app gamit, tapos pag accept sa POS what time nag accept; kasi kahapon created, ngayon accepted sa receiving ng store, today hindi sya nakikita." **Root cause ng "hindi nakikita":** ang dashboard STORE TRANSFER date filter ay `t.created_at::date = @date` — **nag-500** (`operator does not exist: date = text` — walang cast sa Npgsql parameter) kaya laging "No transfers found" kapag may date filter! **Fix + features:** (1) **DB** — `wh_transfers` + **`received_at`** (accept time) + **`created_via`** (app); backfill `received_at = updated_at` para sa completed/partial; (2) **API** — receive → `received_at = NOW()`; create → `created_via` (mobile/dashboard); list response + `receivedAt`/`createdVia`; **date filter → `created_at::date = @date::date OR received_at::date = @date::date`** (may `::date` cast — fix sa 500) → lumalabas ang transfer sa **parehong araw** ng created at accepted; (3) **dashboard STORE TRANSFER panel** — bagong **Accepted** column (oras) + **Via** (📱 mobile / 🖥 dashboard) + CSV columns; (4) **whmobile** — TRANSFER list may **✅ Accepted: <time>** line sa completed/partial + `createdVia: 'mobile'`. **Verified (DEV end-to-end):** transfer #1164 (createdVia=mobile, received_at set) → simulate created kahapon/received ngayon → **kasama sa DATE=today (1) at DATE=yesterday (1)**, hindi sa ibang araw (0) ✓; date filter walang 500 na; cleanup 100% (0 test rows). API **1.1.81** · whmobile `WEB_VER 20260919-13` · dashboard `v=20260919c`. Walang APK rebuild. **NOTE:** "sino ang nag-accept" (`received_by`) = kailangan ng POS client update (v1.1.84) — hindi kasama sa ngayon.

## Latest Change (2026-09-19) — 🧾 Reporting tags + outgoing-only transfers + store choices cleanup + receiving suppliers; API 1.1.80 · whmobile 20260919-12

**Requests:** (1) "sa reporting, all transaction ba naka-tag sa login user?" (2) "bakit meron transfer history sa mobile wala pa sya na transfer?" (3) "tanggalin ang warehouse sa choices" (4) "TEST pang admin lang" (5) "sa receiving paki supplier ang ilagay". **Fixes:** **(1) Reporting tags:** DB +`wh_walkin_sales.cashier_name` + `wh_transfers.created_by` (migrations); API — `/warehouse/sell` + `CashierName` (header + delta trail `user_name`), `/warehouse/transfers` + `CreatedBy`, `/warehouse/receivings` + `UserName` (trail), `/warehouse/sales` response + `cashierName`, transfers response + `createdBy`; whmobile ipinapadala ang `session.fullName || session.username` sa sell/transfer/receiving; sales list item may cashier; **dashboard Mobile POS SALES tab + Cashier column** (+ CSV). **(2) TRANSFER list = outgoing lang** — bagong `direction=out` param sa `/warehouse/transfers` (`from_store_id = @sid` lang); U Got list = **0** hanggang may ma-transfer (dati 195 incoming HQ→U Got ang lumalabas). **(3) Warehouse removed sa store choices** — API login/validate filter + default preference (HQ na ang fallback, hindi Warehouse) + whmobile `visibleStores()`/`pickDefaultStore`. **(4) TEST (STORE-DEV-0001) = admin lang** — API filter (`role == 'Admin'`), whmobile `visibleStores()`, transfer destinations excluded (whmobile + dashboard). **(5) RECEIVING source = SUPPLIERS** — `/dashboard/suppliers` (active) sa dropdown (+ agent label, hal. "Growsari Enterprise Inc. (Jayson)") + "✏️ Iba pa (i-type)". **Verified (DEV end-to-end):** sell 721 → `cashier_name=TEST CASHIER JUAN` + trail `user_name` ✓; transfer 1160 → `created_by` ✓ + `?direction=out` (DEV=1, U Got=0, U Got unfiltered=195 ✓ regression); receiving → trail `user_name` ✓; login filters — abi = HQ/U Got/HVR/ACGS (**walang WH/DEV**), admin = +**DEV** (walang WH) ✓; sales response may `cashierName` ✓; cleanup 100% (0 test rows). API **1.1.80** · whmobile `WEB_VER 20260919-12` · dashboard cache-buster `v=20260919b`. Walang APK rebuild.

## Latest Change (2026-09-19) — 📥 STOCK RECEIVING store-aware sa mobile POS (lahat ng stores); API 1.1.79 · whmobile 20260919-10

**Request:** "the stock receiving gone in mobile pos" + "parehas sa HQ, ang pinagkaiba lang ay ang store na tatanggapin — kung U Got, HQ/HVR/ACGS ang sources; kung HQ, U Got/HVR/ACGS." **Fix:** (a) **Unhide** ang RECEIVING sa store mode (dati nakatago dahil HQ-only ang endpoint — HQ stock ang madadagdagan); (b) **API 1.1.79:** `POST /warehouse/receivings` → `stockSource='store'` + `storeId` (idadagdag sa napiling store + link resolution sa store na iyon + delta trail para i-apply ng store POS locally); `GET /warehouse/receivings` at `/receivings/{ref}/items` → bagong `storeId` param (per-store history); (c) **whmobile:** bagong **Source dropdown** (`recvSourceSel`) — ang IBANG stores (U Got → HQ/HVR/ACGS; HQ → U Got/HVR/ACGS) + "✏️ Iba pa (supplier — i-type)" para sa free text; `searchRecvProducts` → `srcQ()` (store-aware products); `saveReceivingBatch` → `stockSource`/`storeId`; history per store (`recvHistQ()`); auto-load ang sources sa `goReceiving()`. **Verified (DEV store end-to-end):** receiving +5 pcs → stock **10→15** + delta **+5** (pending) + history/items lumabas sa `?storeId=STORE-DEV-0001`; cleanup 100% (0 test rows). Walang APK rebuild — i-reload lang ang app.

## Latest Change (2026-09-19) — 🔒 DEVICE STORE LOCK (i-lock ang store sa phone); whmobile 20260919-9

**Request:** "pwede ba ilock ang store sa cp model?" — para hindi makapagpalit ng store ang staff sa phone (hal. U Got phone). **Feature (web-only, walang APK update):** bagong **SETUP → 🔒 Store Lock** card — `lockStoreToDevice()` (i-lock ang device sa kasalukuyang store, may confirm), `unlockStoreDevice()` (i-unlock, may confirm), `updateLockUI()` (status + button visibility; tinatawag sa `enterMain`). **Enforcement:** `getLockStore()` (`wh_lock_store` localStorage) — (a) `applySavedStore()` → **locked store ang laging piliin** (mas mataas sa `wh_last_store`; may warning kung walang access ang user); (b) `openStoreSwitcher()` → **hindi na bumubukas** kapag naka-lock (toast); (c) login/reopen/validate → auto-enter sa locked store (walang picker). **Verified end-to-end (headless, persistent profile):** RUN 1 = login + pick U Got + lock → status "NAKA-LOCK sa U Got Minimart - Naic", UNLOCK button lumabas; RUN 2 = reopen → `session.storeId=E174` (U Got), walang picker, **switcher hindi bumukas** (`false`) ✓. `WEB_VER 20260919-9`. **Soft lock:** kung i-clear ang app data, mawawala ang lock (para sa hard lock: dedicated per-store user account — existing feature).

## Latest Change (2026-09-19) — 🛒 Cart buttons sa ISANG ROW (SELL NOW | CLEAR | HOLD | HELD); whmobile 20260919-8

**Request:** "sell now clear hold held — isang row" (dati: SELL NOW sa sariling row + CLEAR/HOLD/HELD sa ibaba). **Fix (`whmobile.html` lang):** bagong `.cart-actions` CSS (rem-based — sumusunod sa Display Size selector) — 4 buttons sa isang flex row (SELL NOW `flex 1.4` dahil pinakamahabang label; tanggal ang 🗑/⏸ emoji). **Verified (360px iframe test + headless):** 4 buttons sa **isang row** (lahat y=251), **walang overflow** (rightmost 336 < 360); SELL NOW 91×32, CLEAR 67×32, HOLD/HELD 69×32; JS syntax OK, div balance 0. `WEB_VER 20260919-8`.

## Latest Change (2026-09-19) — 🖨 Standard 57mm font, HINDI bold (web-only ESC/POS, walang APK update); whmobile 20260919-7

**Request:** "change printing font to standard 57mm size, not so bold, so it can occupy more space." **Root cause:** ang native APK (`BluetoothPrinter.kt`) ay **nagpi-print ng BOLD ON sa BAWAT linya** (`ESC E 1` bago ang text, hindi pinapatay) — kaya sobrang bold ang lahat. Ang font ay Font A (12×24) = **32 chars/57-58mm** — standard na ✓ (walang width change). **Fix (web-only — walang APK rebuild):** bagong helpers sa whmobile — `escWrap(t)` (nag-e-embed ng `ESC E 0` = bold-off **pagkatapos ng markers** `<<CENTER>>/<<RIGHT>>/<<BIG>>`, per line, dahil per-line bold-on ang native) + `ptPrint(t)` (= `AndroidPrinter.print(escWrap(asciiSafe(t)))` — asciiSafe muna, ESC bytes hindi dapat salain). **Lahat ng 8 print call sites → `ptPrint`** (receipt, reprint, end-shift, inventory, receiving, online order receipt, pick slip, credit voucher); **`printTest()`** → JS-built test print (dati native `printTest()` na bold pa rin) — may "Bold: OFF" line. **Verified (headless harness, W=32 at W=48):** lahat ng builders **over=0, noEsc=0** (bawat non-empty line may `ESC E 0`, visible length ≤ W); JS syntax OK. `WEB_VER 20260919-7`. **Test:** SETUP → 50mm → 🧪 PRINT TEST + isang sale receipt → dapat **normal weight** na ang text, pareho pa rin ang layout. Fallback kung may printer na hindi sumunod sa ESC E: APK v1.0.18 (native change).

## Latest Change (2026-09-19) — 📱 Mobile POS DEFAULT STORE fix (hindi na bumabalik sa WAREHOUSE/HQ); whmobile 20260919-6

**Request:** "how to change default store kasi balik ng balik sa ibang store — U Got ngani." **Root cause:** (1) ang `whapp/login` at `whapp/validate` ay nagbibigay ng `storeId` na **WAREHOUSE > HQ > first** kapag walang ipinadalang storeId; (2) pagkatapos pumili ng store sa picker, **hindi nire-save ang `wh_session`** (naka-save lang ang `wh_last_store`) → sa bawat reopen, ang validate ay nagpapadala ng **lumang storeId** (WAREHOUSE/HQ) → bumabalik doon ang app; (3) ang `doLogin` ay hindi nagpapadala ng naka-save na store. **Fix (whmobile lang, walang API change):** bagong `applySavedStore(stores)` + `saveSession()` helpers — (a) `doLogin` → ipinapadala ang `storeId: wh_last_store` at kung valid pa ang saved store, **hindi na nagpapakita ng picker** (diretso sa store na iyon; pwede pa ring magpalit sa burger → Store switcher); (b) `renderStores` chip click + `openStoreSwitcher` → nire-resave ang `wh_session`; (c) `init` (no-token path, validate, catch) → `applySavedStore` + `saveSession`. **Verified end-to-end (headless, persistent profile):** Run 1 = login (picker lumabas) → pick U Got → `wh_last_store=E174`, `wh_session.storeId=E174`; Run 2 = reopen → **picker HINDI lumabas**, diretso sa **U Got Minimart - Naic** (`session.storeId=E174`) ✓. `WEB_VER 20260919-6`. Walang APK rebuild.

## Latest Change (2026-09-19) — 🖨 50mm print format fix (lahat ng print builders W-aware); whmobile 20260919-3

**Request:** "yung printing sa 50mm paki ayos format." **Audit findings (32-char/50mm):** (1) `printOrderReceipt` (online order) = **hardcoded 44-char layout** (`center()` 44, `padN(38)`, item line 30+) → basag lahat sa 32; (2) `printReceivingBatch` — `Source: `+fit = 40, `Qty   `+fit = 38 → wrap; (3) `printInventory` — header row = 34 + misaligned STK column (4-digit stock = 33); (4) sale receipt + reprint — `Date : `+petsa ~34, `Cashier : `+name 42; (5) end-shift — `Cashier:` line 41 + `───` prints as `?`; (6) `─`/`•`/emoji → `?` (US-ASCII ang ESC/POS encoding ng APK). **Fix:** bagong shared helpers — `PW()` (32/48), `fitW`, `wrapW`, `padLR` (label+dots+amount, laging ≤ W), `ctrW`, `asciiSafe` (₱→P, box-drawing→`-`, bullets→`*`, emoji/non-ASCII→tanggal) — at **rewrite lahat ng print builders**: sale receipt + reprint, end-shift (+ shift history), inventory (aligned ITEM/STK/`____` columns), receiving voucher, online order receipt (44→PW), pick slip, credit voucher. **Verified:** headless harness (mock data, in-app stubs) — lahat ng builders sa **W=32 at W=48 = 0 overflow lines** (receipt/reprint/endShift/receiving/orderReceipt/pickSlip/inventory); JS syntax OK, div balance 0. `WEB_VER 20260919-3`. Walang APK rebuild — i-reload lang ang app; i-test sa SETUP → 50mm → PRINT TEST.

## Latest Change (2026-09-19) — 📱 Mobile POS COMPACT UI pass (standard mobile sizes + Display Size selector); whmobile 20260919-2

**Request:** "make the mobile POS all mobile friendly — buttons and text standard size for mobile use, ang laki ng lahat, hindi kasya sa viewing." **Changes (`whmobile.html` lang — walang APK rebuild):** (1) **global rem scale** `html { font-size: 13px }` → lahat ng Tailwind sizes/spacing −~19%; (2) **compact custom CSS** — inputs 10×12px padding/14px font, buttons 10–11px/13–14px, cards 11px, qty buttons 34px, cart items 8×10px, menu rows 9×12px, bottom nav icon 18px / 9.5px label, toast 12px, unit chips 11px; (3) **heading overrides** — text-3xl→21px, text-2xl→18px, text-xl→16px; (4) **phone-width container** — `#app` + `.nav-bottom` max-width **560px** centered (hindi na stretched sa tablet/desktop); (5) **SETUP → 📱 Display Size** selector — **Maliit (12px) / Standard (13px) / Malaki (14px)**, naka-save per device (`localStorage wh_ui_scale`). **Verified:** headless Chrome render (login + main SELL screen) — compact at standard mobile sizes, centered, walang layout break (JS syntax OK, div balance 0). `WEB_VER 20260919-2`.

## Latest Change (2026-09-19) — 📱 Mobile POS consolidation: QR-only points + store isolation (VOID fix, transfer filter) + dashboard SALES/END SHIFT tabs; API 1.1.78 · whmobile 20260919-1

**Request:** (1) pag nag-pick ng customer sa mobile POS — nag-e-earn ba ng points? (2) nare-record ba lahat? (3) may sariling endshift report? (4) may sariling sales report sa dashboard? (5) "kung ugot ang store na napili dapat lahat ng ugot transaction makita dyan, report/transfer/endshift — walang mix store". **Audit findings + fixes:** (a) **Points** — dating ang `/warehouse/sell` ay nag-a-award sa KAHIT ANONG customer at `points_per_unit` lang (0 sa lahat ng units → halos laging 0); NGAYON: **QR customers lang** (`qr_code <> ''`), POS-rule (exempt → 0; ppu>0 → ppu×qty; else subtotal/₱200 default mula `store_settings.PointsRate`), response may `pointsPrevious/pointsEarned/pointsNew/qr`; mobile receipt may POINTS section (Previous/+Earned/New o "Walang points account"). (b) **Customer picker** — default **⭐ QR customers lang** (`/warehouse/customers?qrOnly=true`) + **"Lahat" toggle** para sa Credit (non-QR); ⭐ badge + points balance sa results; Credit payment na walang customer = 400 ("Credit payment requires a customer"). (c) **VOID sale store-source fix** — dating store-source sales ay nagre-restore ng stock sa retired `wh_products`; ngayon **tamang store + delta trail** (POS applies via stock-deltas). (d) **EDIT sale** (`PUT /warehouse/sales/{id}`) — same store-source fix. (e) **TRANSFER list** — dating LAHAT ng stores ang nakalista; ngayon `?storeId=` filter (papasok + papaalis lang) + **⬅ IN / ➡ OUT** badges. (f) **Chat 💬** — nakatago na sa store mode (HQ customer service lang). (g) **Dashboard "📱 Mobile POS" panel** (Reports) — tabs **SALES | END SHIFT** per store (HQ/U Got): sales list (invoice/date/customer/payment/items/total) + click → **item detail modal** + range (Today/7D/30D/Month) + totals + CSV; shifts tab = dating end-shift table. **Verified (DEV store end-to-end):** QR customer sell ₱600 → **+3 pts (100→103)**, non-QR → **0 pts**, credit w/o customer → **400**, `qrOnly` filter → QR lang, VOID → stock restored (10-2-2+2=8) + points reverted (100) + delta +2, transfer filter per store (DEV 4 / U Got 192 / HQ 756 vs unfiltered 1062); cleanup 100% (0 test rows). API **1.1.78** · whmobile `WEB_VER 20260919-1` · dashboard cache-buster `v=20260919`. **U Got POS v1.1.83 update:** pending pa rin (kailangan nakasara ang app; exe ready sa `C:\JumongAPI\client\` + `wwwroot/updates/JumongPosV1.01_1183.exe`).

## Latest Change (2026-09-19) — 📱 Store-aware Mobile POS (U Got) — SELL + TRANSFER + END SHIFT per store; API 1.1.77 · POS v1.1.83 (delta pull all stores)

**Request:** isang "Jumong Pos" app lang, store/user ang difference — U Got mobile POS na naka-connect sa U Got stock: SELL (sariling benta), INVENTORY, TRANSFER OUT (U Got → ACGS/HVR/HQ), at END SHIFT per store na nakikita sa dashboard. **API (DashboardController, → 1.1.77):** (a) `GET /warehouse/products` + `/inventory-summary` → bagong `source=store&storeId=` (per-store stock/units/prices, same shape as HQ); (b) `POST /warehouse/sell` → `stockSource='store'` + `storeId` (deduct sa store, `wh_walkin_sales.stock_source=<storeId>` para HIWALAY ang benta, stock_trails delta pos_id<0 para i-apply ng store POS); (c) `wh_transfers.from_store_id` + `source='store'` — create validates source-store stock, receive deducts source store (+ delta trail), `GET /transfers/{id}/items` at list may fromStoreId; gumagana ang destination receive ng lahat ng stores incl. **HQ** (client id 5); (d) `wh_daily_closes.store_id` (migration: existing 11 rows → HQ) — end-shift preview/save/list per store (`stock_source` filter; last-close per store) + `warehouse/sales`/`sales/summary` storeId filter. **whmobile.html (WEB_VER 20260918-1):** `srcQ()`/`isHqStore()` helpers — lahat ng SELL/INVENTORY/PRODUCTS/TRANSFER search ay sumusunod sa napiling store; TRANSFER destinations kasama ang HQ kapag store source; SELL payload stockSource/storeId; END SHIFT may storeId; per-store sales/shift lists; HQ-only menu items (Online Orders/Receiving/Credit Billing) nakatago sa store mode. **POS client v1.1.83:** `PullStockDeltasAsync` — inalis ang HQ-only guard → LAHAT ng stores nag-a-apply ng server deltas (store mobile SELL/TRANSFER → bumababa ang local stock within 10s). **Dashboard:** bagong **📱 Mobile POS Shifts** panel (Reports) — per-store sections (MOBILE POS HQ / MOBILE POS U GOT): date/cashier/sales/cash/e-wallet/credit/expenses/cash-on-hand/difference + totals + CSV; cache-buster v=20260918. **Verified (DEV store end-to-end):** SELL 2 pcs → stock 10→8 + sale (stock_source=STORE-DEV-0001) + delta -2; TRANSFER DEV→HQ 3 pcs → receive completed → stock 8→5 + delta -3; END SHIFT DEV → preview 1 sale ₱20 → save → `shifts?storeId=DEV` may storeId, HQ list hindi apektado; cleanup 100% (0 test rows). **U Got POS update sa v1.1.83:** pending — kailangan nakasara ang app (running ngayon); exe naka-ready sa `C:\JumongAPI\client\` + `wwwroot/updates/JumongPosV1.01_1183.exe` para sa agent update+restart.

## Latest Change (2026-09-17) — 🛠 Cloudflare tunnel: duplicate connector fix (2× cloudflared → 1)

**Reported:** intermittent `ERR_CONNECTION_CLOSED` sa dashboard polls (`warehouse/transfers/pending-count`, `shop/orders/new-count`, `checks/words`, `chats/unread`) — pareho ng dating Sep 6 symptom. **Diagnosis:** API service = healthy (20h uptime mula sa 09-16 deploy, walang crash; live tests 10/10 + 8/8 = 200; walang network errors sa System log). **Root cause:** **DALAWANG cloudflared tunnel connectors** na tumatakbo mula 14/09 19:57:57 boot — parehong boot trigger ang nag-start: (1) `cloudflared-tunnel` (SYSTEM, direct `cloudflared.exe tunnel --config ... run` — ito ang documented/current) at (2) `JumongCloudflareTunnel` (legacy, `wscript C:\JumongAPI\cloudflare_tunnel.vbs` → parehong command) → duplicate connector (posibleng stale/zombie) → paminsan-minsang connection resets. **Fix (server config lang):** `Disable-ScheduledTask JumongCloudflareTunnel` + kill ang orphan cloudflared PID 9196 (keep PID 2424 mula sa `cloudflared-tunnel`). **Verified:** 1 cloudflared process na lang, `cloudflared-tunnel` Running, public URL 8/8 OK, lahat ng dating failing endpoints 200. (Tailwind CDN warning = known/accepted, internal dashboard.) Walang code change.

## Latest Change (2026-09-16) — 🚫 Master-INACTIVE products (53) hindi na lumalabas sa TRANSFER/SELL pickers (mobile + dashboard dropdown)

**Request:** "dapat di nakikita ang inactive sa mga transfer product" — ang mga product na na-deactivate sa master catalog (pero may stock pa: 555 SARDINES TOMATO SAUCE 74, TATTOOS CHEDDAR CHIPS 56G 25, Absolute Pure Distilled 8L 24, MALING CLM 198G 12, RSL Cracker Spicy/16.45oz/32oz, PUREFOODS CS LUNCHEON MEAT SAVE 6 3, DOVE HAIR FALL RESCUE 3) ay lumalabas pa rin sa mobile TRANSFER/SELL search at dashboard Store Transfer dropdown. **Root cause (verified sa cloud PG):** ang pag-deactivate sa master ay HINDI nagpo-propagate sa cloud `products` rows (by design — para manatili ang dead stock sa inventory value/cost reports); ang mobile (`/dashboard/warehouse/products?source=hq`) at dashboard dropdown (`/dashboard/stock-status?storeId=HQ`) ay `products.is_active` lang ang filter. HQ breakdown: **750 active products = 695 may active master + 53 puro-INACTIVE master + 2 legacy na walang master**; **0 barcode** na may mixed active/inactive master rows. **Fix (picker-level lang — WALANG data/report/POS change):** bagong optional **`excludeMasterInactive`** param (default OFF) sa dalawang endpoints — predicate: `COALESCE((SELECT bool_or(mp.is_active) FROM master_products mp WHERE mp.barcode <> '' AND mp.barcode = <row>.barcode), TRUE)` (walang master match = panatilihin / legacy; may active = panatilihin; puro inactive = itago). Ginagamit lang ng: whmobile **TRANSFER** search (`searchTxProducts`) + **SELL** search (`searchProducts`) at dashboard **STORE TRANSFER** product dropdown (index.html) — ang mobile INVENTORY/PRODUCTS/RECEIVING, dashboard VIEW STOCK/TOTAL (components.js), at POS ay HINDI ginalaw. **API version → 1.1.76** (2 constants: `/version` + agent heartbeat) · whmobile `WEB_VER 20260914-1 → 20260916-1`. **Verified live:** `warehouse/products?search=555 SARDINES&source=hq&excludeMasterInactive=true` → **1 row** (active chili variant 748485200026), walang param → **2 rows** (unchanged); `stock-status?storeId=HQ&excludeMasterInactive=true` → **697** vs **750** (diff=53), **0 leaked** sa 10 problem barcodes, 10 pa rin sa unfiltered (regression OK). Deployed sa server (publish + WinRM copy + service restart). **Cache:** dashboard hard-refresh (Ctrl+F5) para sa index.html; whmobile WebView reload. Walang client release, walang data change.

## Latest Change (2026-09-16) — 🧹 Phantom store cleanup (88AF + DEV leak) + BARCODE-SYNC fix (master → stores) + guards

**Request:** (1) alisin ang phantom store na `STORE-20260912-88AF` (737 products, 0 stock — galing sa Sept 12 fresh-DB dev test) na lumalabas sa VIEW STOCK/DEDUCT STOCK; (2) ayusin ang "nawala ang stock count" kapag pinalitan ang barcode sa master (WINGS CALAMANSI incident). **Cloud PG cleanup:** `DELETE FROM products WHERE store_id IN ('STORE-20260912-88AF','STORE-DEV-0001')` → **DELETE 740** (737 phantom + 3 DEV leaked: Alfonso Sherry Oak 6pcs, Alfonso Brandy 7pcs, Camel Yellow) — 4 real stores na lang sa `products` (7159/AA36/E174/A80C). **Guards:** (a) `GET /dashboard/stock-status` → registered stores lang (`p.store_id IN (SELECT store_id FROM stores)`) — hindi na lalabas ang phantom/fresh-DB store ids; (b) `GET /dashboard/summary` → same filter sa `total_products`; (c) `POST /api/products` (SyncController) → **DEV store push disabled** (`STORE-DEV-0001` → `{ok,skipped}`) para hindi na makapasok ang dev stock kahit lumang build. **BARCODE-SYNC fix (root cause ng "nawala ang stock"):** pag pinalitan ang barcode sa master, ang dashboard ay naghahanap ng stock **by barcode** — ang mga POS ay may LUMANG barcode pa hanggang sa susunod na master pull (≤5 min) + snapshot push (30s) → lumalabas na 0 ang TOTAL/SERVER STOCK sa window na iyon. Ngayon: (1) `PUT /products/master/{id}` → kapag nagbago ang barcode, agad na `UPDATE products SET barcode=@new WHERE barcode=@old` sa LAHAT ng stores (savepoint-protected, hindi sinisira ang transaction kung may conflict); (2) `POST /dashboard/warehouse/stock-snapshot` → **master-preferring** na ang isinusulat na name/barcode (LATERAL name fallback + `EXCLUDED.name/barcode` sa ON CONFLICT) — kahit luma ang barcode ng POS push, hindi na nire-revert ang bagong barcode bago pa ang pull. **Verified live:** 88AF/DEV wala na sa `/stock-status` (4 stores, 3007 rows) at `/summary` (3011 products); DEV push test → `{ok:true, skipped:1}` + walang na-insert; propagation test sa product 19 (Alibaba Sweet Corn XS) — PUT temp barcode → 4 cloud rows updated agad, PUT back → 4 rows reverted (walang bakas); snapshot stale-barcode test (ACGS WINGS, `8998866610926` na luma vs master `8998866801614`) → **master barcode ang nanatili** (hindi na-revert). **HISTORY ng WINGS incident:** master 585 barcode `8998866610926` → `8998866801614` (11:58 AM); HQ stock **167** (receiving +168 Sept 15 11:49, adjustment −1 Sept 16 11:40 "alam ni kuya") — HINDI nawala ang stock, transient 0 lang sa dashboard bago mag-sync ang mga POS. **NOTE:** ang master 585 name ay may stray " by" sa dulo (na-edit 12:21 PM; gayon din ang master 31 "Ariel Downy Floral Passion by") — kailangan i-correct sa Master Products editor kung typo. May 1 sale + 2 stock trails pa ang DEV store sa cloud (hindi kasama sa cleanup; optional alisin).

## Latest Change (2026-09-16) — 📤 HQ OUTFLOW dashboard + 📜 Driver End-Shift History (per-day remittance) + Inventory Cost TRUE variance

**Request:** (1) makita sa dashboard ang daily e-commerce sales at transfers out sa ibang stores (HQ only) — para malaman kung saan lumalabas ang stock; (2) daily remittance ng driver — per-day end-shift history sa driver app AT dashboard report; (3) linawin ang negative variance sa Inventory Cost report. **API (DashboardController):** bagong `GET /dashboard/hq-outflow?from=&to=` → `{totals, byStore, byDay}` (e-com delivered orders/total mula `online_orders`; e-com holds cost mula `SHOP-%` trails; transfers out pcs/cost mula `Transfer #%` trails, per-destination mula `wh_transfers` via `Transfer #<id>` parse); `GET /dashboard/shift-history` → bagong `transfersOutCost` + `ecomHoldCost` per close window (prev close → close, cost-valued sa current cost); `GET /dashboard/remittances` → bagong `shiftHistory` (per-day driver end-shift, huling 60 araw); `GET /dashboard/driver/endshift-info` → bagong `history` (huling 30) para sa driver app HISTORY; lahat ng `endedAt` ay PH local na (`.ToLocalTime()` — dati UTC ang display ng closedShifts). **Dashboard (components.js + index.html):** bagong **📤 HQ OUTFLOW** panel (sidebar POS CLIENT → Reports → HQ Outflow) — cards (E-com Sales delivered, E-com Holds stock-out, Transfers Out, Total Stock Out), **per-destination chips** (ACGS/Naic/HVR), per-day table, range (Today/7D/30D/This Month) + CSV; **INVENTORY COST REPORT** — bagong columns **Transfer Out / E-com Hold / True Var** (`trueVar = variance + transfersOut + ecomHold` = tunay na shrink/loss lang, hindi na kasama ang transfers/e-com); **REMITTANCE panel** — bagong **📜 DRIVER END-SHIFT HISTORY** table (Date/Driver/Orders/Cash/GCash/Total Remitted/Ended At + 30D/60D/ALL + CSV + totals row). **Driver app (v2.0.9 native APK):** bagong **📜 END-SHIFT HISTORY** button sa main screen → per-day list (petsa, orders, cash, gcash, total, oras) + TOTAL na na-remit (huling 30); `versionCode 11` / `versionName 2.0.9`; build + sign sa SERVER (gradle 8.14.3 + jumong_sign.keystore, cert SHA-256 `6a107668...` = pareho ng lumang APK — installable ang update); APK → `C:\JumongAPI\wwwroot\updates\JumongDriver.apk` + repo/publish updates + `driver-version.json` 2.0.9. Web driver pages (`driver-app.html` + `driver.html`, magkapareho) updated din (HISTORY screen). **POS client (para sa susunod na release — hindi pa naka-release):** `EndShiftForm` — bagong on-screen **STOCK OUTFLOW** card (Transfers out / E-com holds / Mobile / Manual cost + server channel numbers async — e-com delivered orders/total, COD collected, remitted, transfer pcs); published sa `C:\dev\out\client`. **Verified live:** hq-outflow (Sept 15: transfers ₱143,105 = eksaktong tugma sa end-shift `TotalAdjustDown`; e-com 11 orders ₱19,577.50; Sept 16 may 2 transfers ₱99,905 → ACGS); shift-history new cols (Sept 13: transfers ₱11,047,357.72 — malaking transfer pala kaya malaki ang variance); remittances shiftHistory (kenneth, 6 shifts, PH times na); dashboard files live 200 + `hqOutflow` present. **GOTCHA (naayos):** ang `DashboardController.cs` ay **66MB** na (may mga lumang corrupted/mojibake na comment lines — mula pa sa lumang session) — **HUWAG i-edit gamit ang `Get-Content -Raw` + `WriteAllText`** (nag-mo-mojibake at lumobo sa **765MB** → `CS8103: Combined length of user strings...`); gamitin ang `edit` tool. Restored mula sa git + ni-re-apply ang lahat ng edit (0 errors).

## Latest Change (2026-09-14) — 🏆 ONE RULE: customers ay sa E-COMMERCE registration lang + ⭐ Points Liability sa dashboard + CustomerChatForm FIX (buong pangalan + tanggal ang overlay sa SEND) — v1.1.82

**Request:** (1) i-delete/disable na ang mga manual customer accounts — inactive, walang use (NO delete para safe ang references); (2) makita sa dashboard (Customers panel) ang **current total points** para ma-set aside sa bank; (3) CustomerChatForm — buo pa rin dapat ang mga pangalan, at may word (debug label) na naka-overlay sa SEND button. **CLEANUP (one-time, lahat ng machines):** cloud PG — **350 manual accounts** (`google_sub=''`) → `is_active=FALSE` + `loyalty_points=0` (resulta: **active manual=0, manual points=0**; e-commerce active points **live ~1,005** sa 90 customers, inactivePoints=0); local via agents — HQ **340**, HVR **341**, Naic **341**, ACGS **341**, DEV (out\client, direct SQLite) **330** zeroed/deactivated (`IsActive=0, LoyaltyPoints=0 WHERE QrCode=''`); stale dev root DB (pre-Qr schema) — points zeroed. **API (DashboardController):** bagong `GET /customers/points-summary` → `{activeCustomers, activePoints, inactivePoints, valuePerPoint:1, value}` (active = `is_active AND google_sub<>''`); `POST /customers/deactivate-non-star` → **zero points din** (`loyalty_points=0` kasama ng deactivate). **Dashboard (index.html + components.js, `v=20260914a`):** ⭐ **POINTS LIABILITY (LIVE)** strip sa itaas ng customers table — `pts · ₱ · customers (1 pt = ₱1) · i-set aside sa bank` + note kung may inactive points; refresh kasama ng panel. **POS (v1.1.82):** (a) `CustomersForm` → **VIEW-ONLY** — inalis ang + NEW/✚ SAVE/✖ DELETE/CLEAN DUPES, fields ReadOnly, note "🔒 Read-only — e-commerce registration lang"; (b) `CustomerService.Save` → **UPDATE-ONLY** (may error kung Id=0 — bawal mag-insert); (c) `DataExporter.Import` → customers HINDI na kasama sa delete/insert; (d) **CustomerChatForm FIX**: pangalan — **AutoSize=true + MaximumSize** (gaya ng message bubbles; ang AutoSize=false ay HINDI nag-wrap — dati pinuputol pa rin ang pangalan) + dynamic card height (nameH+54); **DebugHelper.AddFormLabel → `#if DEBUG`** (ang "CustomerChatForm" debug label ay naka-BringToFront sa Bottom|Right — EKSENA naka-overlay sa SEND button sa RELEASE builds; inalis sa lahat ng 23 forms); bubble meta clamped (hindi mag-overflow ang mahabang reply_by name) + thread header AutoEllipsis + tooltip. **Cloud (SyncController/DashboardController):** `SyncCustomers` → **NEVER INSERT** (UPDATE-only sa `google_sub<>''`; ang luma/mali na POS push ay hindi makakagawa ng customer); Google login callback → **LINKING** (email → name, bago gumawa ng "Name 2") — ang manual account na mag-register online ay magiging isang account lang (google_sub + qr_code + active). whmobile: "Tap to select or add customer" → "Tap to select customer" (WEB_VER 20260914-1). **SPLIT FIX (v1.1.82 asset re-uploaded):** ang `SplitterDistance=420` sa object initializer ay **naka-clamp** bago pa ma-parent ang SplitContainer (default small size) + `FixedPanel.Panel1` → **permanenteng napiit ang conversation list (~150px)** at nagugulo ang cards — fix: `Shown +=` ang mag-set ng `SplitterDistance=420` (`if (split.Width > 420)`). Walang store pang nakakuha ng 1.1.82 (HQ 1.1.81, iba 1.1.80) kaya in-delete + re-created ang release v1.1.82 gamit ang fixed exe (walang version bump). `AppVersion` → **1.1.82** · API `latestVer` → 1.1.82 · GitHub release **v1.1.82** · API redeployed (DLL + wwwroot) · client drop → `C:\JumongAPI\client\`. Stores (1.1.80/1.1.81) → UPDATE APP.

## Latest Change (2026-09-12) — 💬 Chat upgrades (v1.1.81): readable names sa POS chat, badge fix, at CHAT sa MOBILE (whmobile)

**Request:** (1) buo dapat mabasa ang customer names sa chat panel (walang photo ang clients — pangalan ang identifier; dati napi-putol at kailangan i-drag ang splitter); (2) may indicator sa 💬 CHAT button kapag may bagong chat; (3) magkaroon din ng chat ang **POS mobile** (whmobile), hindi lang HQ POS. **POS client (v1.1.81):** (a) `CustomerChatForm` — `SplitterDistance` 330→**420** + `Panel1MinSize 320`; **2-line name** sa conversation cards (name label height 18→30, card 88→96, shift contact/last/badge pababa) + **ToolTip** (buong pangalan + phone); (b) `SalesForm` — **CHAT BADGE FIX** (`_lblChatBadge.BringToFront()` pagkatapos ng buttons; dati naipit sa likod ng buttons kaya hindi lumalabas ang red unread count). **whmobile.html (WEB_VER 20260912-1, deployed):** 💬 **floating chat button** (may red unread badge, poll `/dashboard/chats/unread` 10s) + **chat overlay** (conversation list: name/phone/subdivision/last msg/unread → thread: messages + composer; endpoints `/dashboard/chats`, `/chats/{id}/messages`, `/chats/{id}/reply` na may `replyBy = session.fullName||session.username`, `/chats/{id}/seen`; poll 4s habang bukas; mark-seen sa open) — web-only (walang APK rebuild, WebView lang). `AppVersion` → **1.1.81** · API `latestVer` → 1.1.81 · GitHub release **v1.1.81** (id 388222533, asset 562905076, 211,711,420 bytes, download 200) · client drop → `C:\JumongAPI\client\` · whmobile.html → `C:\JumongAPI\wwwroot\` · API redeployed. Stores (1.1.78) → UPDATE APP.

## Latest Change (2026-09-12) — 🚀 POS v1.1.80 RELEASED + end-shift fixes (F3, ewallet list net, phantom opening fund) + HQ historical correction

**Released:** AppVersion 1.1.80 · GitHub release **v1.1.80** (id 387570976, asset 559333179, 211,711,420 bytes, download 200) · client drop → `C:\JumongAPI\client\` · API `latestVer` 1.1.79 → **1.1.80** (redeployed). Stores (1.1.78) → **UPDATE APP** (banner sa POS). **Laman ng 1.1.80:** (1) SELECT CUSTOMER FIRST sa bawat bagong cart (after sale + initial load; cancel = WALK-IN); (2) ⭐ online-registered lang sa customer picker; (3) Customer bar fix: buong pangalan + points (name+points una, AutoEllipsis + tooltip), walang ● dot/white overlay, walang duplicate CUSTOMER chip (BringToFront + padding); (4) **F3 FIX** — `RefreshCart` ay nag-se-select ng row 0 (bagong punch) → ang F3 ay nag-e-edit ng bagong item, hindi ang lumang current row; (5) **E-WALLET LIST NET FIX** — `GetGcashTransactionsSinceLastClose` + `GetGcashTransactionsBetween` ay item-level net na (`SUM(si.TotalPrice WHERE si.IsVoided=0)`) — tugma na sa summary (HVR Sep 10: list 2,669.50 → 2,019.50 ✓, ang ₱650 voided Redhorse sa `INV-AA36-20260910-0103`); (6) **PHANTOM OPENING FUND FIX** — `ShiftSessionService.EndSession()` ay nagre-reset ng `ShiftOpeningBalance` = 0.00 pagkatapos ng bawat close (dating nagma-migrate sa susunod na shift); (7) 💬 CHAT sa dev PC (STORE-DEV-0001 kasama sa `ChatVisibleHere`). **HISTORICAL CORRECTION (HQ pos 214/215):** ang phantom 6,200 opening fund ay nagdulot ng negatives — **pos 214 (Sep 11 21:14): −7,150.25 → opening 0, diff −950.25**; **pos 215 (Sep 12 08:05): −6,256.25 → opening 0, diff −56.25** — corrected sa **cloud PG + HQ local DB** (may `[FIX 2026-09-12]` note). Verified formula: 80,850+58,918−139,327.25−1,391−6,200 = −7,150.25 ✓ exact; 17,000−17,056.25−6,200 = −6,256.25 ✓ exact.

## Latest Change (2026-09-12) — 👤 POS 1.1.80 (dev test): customer info display FIX (buong pangalan + points kita) + picker column widths

**Issue:** pagkatapos pumili ng customer, hindi kita ang **current points** (at minsan ang buong pangalan) — ang format ay `Name · phone · Credit · Points` (points sa DULO, na-auto-clip ang label dahil walang AutoEllipsis/AutoSize=false). **Fix (`SalesForm.cs`):** `UpdateCustomerDisplay` — **pangalan + points muna**: `⭐ Name · Pts: N · Credit: ₱X · phone`; `lblCustomerInfo` → `AutoSize=false` + `AutoEllipsis=true` + **ToolTip** (buong detalye + online-registered note sa hover). **Fix (`SelectCustomerForm.cs`):** `AutoSizeColumnsMode` Fill → **None** + explicit widths (Name 270 · Phone 100 · Address 110 · Credit 75 · Points 70 · Id 35) para **laging kita ang buong pangalan at points**; `CellToolTipTextNeeded` para sa mahabang pangalan/address. **LAYOUT CLEANUP (parehong 1.1.80 dev build):** inalis ang **● dot icon** (`lblCustIcon`) at ang **duplicate CUSTOMER/WALK-IN chip** (`lblOrderChip`) sa customer bar — ang status ay nasa info line na lang (`⭐ Name · Pts...` / `🧾 WALK-IN`); `lblCustomerInfo.Padding` → 12 (text malapit sa kaliwa, walang overlap); `BringToFront` sa 3 buttons (CUSTOMER/CHAT/SCAN QR) — walang puting overlay; inalis ang conflicting manual bounds sa resize. Published sa `C:\dev\out\client` (local; hindi pa nai-deploy — pending dev test + release). NOTE: ang POS Promo dark panel (kanan) ay normal feature (dashboard SETTINGS → 📢 POS Promo); ang dev DB ay may ⭐ customers (verified `AS-`), para sariwain: Settings → SYNC CUSTOMERS.

## Latest Change (2026-09-12) — 👥 CUSTOMERS: ⭐ (online-registered) default sa dashboard + i-deactivate ang non-ecommerce customers

**Request:** "make star customer default" + "those not registered account thru ecommerce make it inactive." **API (DashboardController):** `POST /customers/deactivate-non-star` (UPDATE `is_active=FALSE` WHERE walang qr_code; returns count) + `POST /customers/{id}/activate` (restore single). **Dashboard (components.js + index.html, `v=20260912a`):** `ptsFilter` default → **`star`** (⭐ list agad pagbukas); i-hide ang inactive sa list/counts (may **"may inactive"** toggle + count); **"⬇️ DEACTIVATE W/O ⭐"** button (confirm + count toast); inactive rows dimmed + **INACTIVE** badge + **"↩️ BALIK"** restore action; ang counts (`withStar`/`withoutStar`/`noPhone`) ay active lang. **RUN:** na-deactivate ang **347** non-⭐ (resulta: `active_with_qr=186`, `active_without_qr=0`, `inactive_total=349`). **NOTE:** ang 13 non-⭐ na may credit balance (hal. `inventory hq` ₱259k, `Boss Nelson` ₱243k, `EXTRAMORE` ₱67k — internal/wholesale accounts) ay na-deactivate rin — pwede i-restore via "↩️ BALIK"; ang 69 non-⭐ na may 3,660 legacy points ay HINDI na-zero (history). POS enforcement: ang bagong **1.1.80** ⭐ picker filter ang nagtatago sa kanila; para i-enforce sa stores, i-release ang 1.1.80 (naka-pending sa dev test).

## Latest Change (2026-09-12) — 👤 POS v1.1.80: SELECT CUSTOMER FIRST sa bawat bagong cart + ⭐ (online-registered) lang sa customer picker — DEV TEST (hindi pa release)

**Request:** "select customer muna every new cart" + "alisin sa customer selection ang walang star na customer na hindi nag-register online." **A (`SelectCustomerForm.cs`):** ang picker ay **⭐ lang** (may QR / online-registered) + "🧾 WALK-IN (no points)" row; ang manual POS customers na walang points ay hindi na lalabas (filter sa `!string.IsNullOrEmpty(QrCode)` sa parehong GetAll at Search); header: "SELECT CUSTOMER — ⭐ online-registered lang · TOP = WALK-IN". **B (`SalesForm.cs`):** bagong `PromptNewCartCustomer()` (reuse `AttachCustomer`) — nagpa-prompt ng customer picker sa **`OnShown` (initial load)** at **pagkatapos ng bawat completed sale**; Cancel/Escape = WALK-IN; mananatili ang change-customer-anytime (customer button/customer bar). `AppVersion` → **1.1.80**. **Publish: `C:\dev\out\client` (LOCAL ONLY — hindi pa nai-deploy sa server; pending ang release hanggang ma-test sa dev).** Dev run: ang plain file copy ng WAL-mode dev DB ay **"database disk image is malformed"** → gumamit ng **SQLite Backup API** (temp .NET 8 console, System.Data.SQLite.dll mula bin) → malinis na kopya ng `JumongPos.db` (STORE-DEV-0001) sa out\client → POS nag-start ng maayos (Login screen). TODO: user test sa dev → release (drop + UPDATE APP) + final changelog.

## Latest Change (2026-09-11) — 🏦 CHECKS panel: DUE grouping (PAST DUE / DUE TODAY / COMING DAYS by withdrawal date) + per-group & grand totals

**Request:** paghiwalayin ang due today at coming days sa dashboard Checks, ngunit **laging pinagsama ang Sabado/Linggo/Lunes** (Lunes ang check withdrawal ng bangko), may totals. **Frontend-only (components.js + index.html, cache-buster `v=20260911a`):** bagong `dueSections` getter sa `checksPanel` — **effective withdrawal date** per check (petsang Sat/Sun → susunod na Lunes; Mon → Lunes mismo; iba → sariling araw); sections: 🔴 **PAST DUE** → 🟡 **DUE TODAY** → 🟢 **COMING DAYS** per-day (Monday group label "DUE MON — kasama ang Sat·Sun·Mon"), bawat section may **count + total**; **GRAND TOTAL** footer (₱ + count). Sumusunod sa status/search filter (per user choice). **Verified live** (grouping logic vs totoong data, 15 checks): PAST DUE 2/₱1,435,660 · DUE TODAY 1/₱433,362.40 · DUE MON 6/₱1,516,670.79 · DUE WED 1/₱332,000 · DUE FRI 2/₱196,258.98 · DUE MON (susunod) 3/₱1,382,084 · **GRAND ₱5,296,036.17**. Deploy: wwwroot copy lang (walang API change). **FIX (v=20260911b):** duplicate `:key` ang dalawang DUE MON sections (parehong title) → nag-skip/merge ang Alpine kaya hindi lahat ng check nakikita — nagdagdag ng unique `sec.key` (past/today/date) sa `mk()` at `:key="sec.key"` sa index.html; verified 15/15 checks sa 6 sections na may unique keys. **PAYABLE TOTALS (v=20260911c):** default status filter = **Issued**; ang per-section count/total at **GRAND TOTAL (PAYABLE)** ay **issued lang** ang binibilang — hindi na kasama ang **cleared/void** (hal. ang void ₱1M check ay hindi na sa DUE MON total); ang cleared/void ay makikita pa rin sa status selector (may "👁 HISTORY VIEW — hindi kasama sa payable total" note kapag non-issued ang filter). Verified live: 13 issued = **₱3,697,906.68** payable (2 void rows excluded sa totals). **COLUMN ALIGNMENT (v=20260911d):** dating bawat section ay sariling `<table>` kaya magkakaiba ang column widths — pinagsama sa **ISANG table** na may section header rows (`<td colspan="9">` na may colored strip) + `whitespace-nowrap` sa fixed columns (Due Date/Check No./Bank/Amount/Status/Created/Printed/Actions); pantay na ang columns sa lahat ng sections. **FIX RENDER (v=20260911e):** ang nested `x-for` (section header + item rows sa iisang table) ay hindi nag-render ng individual rows (headers/totals lang ang lumabas) — ibinalik sa **per-section tables** (proven working) na may **`table-fixed` + `<colgroup>`** (same % widths sa lahat ng sections) para pantay pa rin ang column alignment; `truncate` sa Payee para hindi masira ang layout sa mahabang pangalan. **FIX CLEARED 404 (v=20260911f):** ang ✔ CLEARED button ay nag-call ng `/api/checks/{id}/cleared` pero ang endpoint ay `/clear` → **404** — inayos ang JS (`'cleared' → 'clear'`) AT nagdagdag ng alias route `[HttpPost("{id:int}/cleared")]` sa controller (gumagana kahit luma ang cached UI). Verified: POST `/cleared` at `/clear` → ok (temp check test, deleted). **RESPONSIVE ACTIONS (v=20260911g):** sa maliit na browser, hindi makita ang Actions buttons (kanan) dahil `table-fixed` + percentage widths at `overflow-hidden` sa cell — binago sa **fixed px column widths** (1190px total, Actions 300px) sa loob ng **`overflow-x-auto`** (horizontal scroll) para laging maabot ang PRINT/✏️/CLEARED/VOID; inalis ang `overflow-hidden` sa actions cell; pantay pa rin ang columns sa lahat ng sections.

## Latest Change (2026-09-11) — 🛒 E-COMMERCE: FAIL-CLOSED stock check — hindi na makaka-order ng walang stock

**Issue (HQ cashier):** "bakit nakaka-order ang online customer kahit out of stock?" **Root cause:** sa `AdjustOrderStockHeld` (stock HOLD sa `ShopCreateOrder` at item-edit), ang item na (1) walang master record, (2) blank barcode, o (3) walang ACTIVE HQ product row ay **tahimik na ni-skip** (`continue`) → walang shortage → **na-approve ang order kahit walang stock**. Confirmed sa data: `SHOP-20260831-0093` WAWANG MOSQUITO COIL x2 — walang HOLD trail (inactive ang `...969` HQ row noon; na-skip). Ang out-of-stock na may ACTIVE row ay tama na ang block (guarded `stock_qty >= pcs`). Aug 22-26 orders = walang hold dahil na-deploy lang ang HOLD feature Aug 27 (v1.1.60). **Fix (DashboardController `AdjustOrderStockHeld`, RESERVE path lang):** fail-closed — item na walang master record / blank barcode / delisted (`is_active=false` o `sell_online=false`) / walang active HQ row → idinagdag sa shortages → `BadRequest` → rollback → **walang order**; malinaw na mensahe ("wala sa master catalog", "walang barcode", "hindi na available", "walang active HQ stock record"). Ang RELEASE path ay HINDI binago (para hindi ma-block ang pagbabalik ng stock sa cancel/edit). **Fix B (shop.html):** `placeOrder()` → `refreshCatalog()` (fresh fetch, walang 5-min localStorage cache) bago mag-submit → i-aalis/adjust ang cart items na out of stock o wala na sa catalog → toast + abort (server pa rin ang final gate). **Verified live (test session cookie + 3 cases):** (a) fake productId 999999 → **400** `Product #999999 (wala sa master catalog)`; (b) zero-stock id 19 (Alibaba Sweet Corn XS) → **400** `PC x1 (4809013044553)`; (c) in-stock id 1 (555 Tuna Adobo) → **200** SHOP-20260911-0310 + HOLD trail `-1 | 168 → 167`; test order/trail/session deleted, stock restored 168, sequence reset. Deploy: API DLL + shop.html (live 200). NOTE: linked-child products (Alfonso by 12, Coke by 12, atbp.) ay VALID na order — ang stock ay nasa parent, kaya ang child ay laging 0 sa POS (hindi ito bug).

## Latest Change (2026-09-10) — 🏦 CHECKS app (EastWest + RCBC): recording + server-side printing sa HP Smart Tank

**Request:** check recording + printing (bank, check no mula sa user, payee mula sa suppliers, date, amount; ipi-print ang payee/date/amount/amount-in-words) + EastWest format guidelines (date MM-DD-YYYY sa pre-printed boxes, ₱/PESOS pre-printed, amount #,##0.00, isang payee lang, words nagtatapos sa "only") + RCBC bilang pangalawang bank (EastWest default) + created date sa record. **DB:** `checks` (bank/check_no/check_date=due date/supplier_id/payee/amount/amount_words/memo/status issued-cleared-void/printed_at/print_count/created_by/created_at; UNIQUE bank+check_no) + **PER-BANK `check_template`** (X/Y mm calibration, `date_mode` boxes|line, `date_format`, printer/paper). **API (`ChecksController` `/api/checks` + `CheckPrintService`):** CRUD (bank validation, supplier-only payee, unique check no per bank, auto amount-in-words), `/{id}/void` + `/{id}/clear`, `/{id}/print` (template ng bank ng check; stamp printed_at/print_count), `test-print?bank=`, `template?bank=` GET/PUT, `banks`, `printers`, `words`; layout = date digits sa boxes via pitch o single-line, payee/amount/words shrink-to-fit, custom paper size, **explicit printer** (default ng server ngayon ay XP-80C thermal — dapat i-specify ang HP). **Dashboard:** POS CLIENT → 🏦 Checks — form (Bank dropdown EastWest default/RCBC, Check No, **DUE DATE**, Payee=Suppliers dropdown, Amount, auto words preview, Memo), list (Due Date · Check No · **Bank** · Payee · Amount · Status · **Created** · Printed) + PRINT/REPRINT/CLEARED/VOID/EDIT + search/status filter; ⚙️ per-bank Print Position Settings + TEST PRINT. Cache-buster `v=20260910b`. **Verified live:** banks [EastWest Bank, RCBC]; EastWest calibration preserved (date 149/10 · 161/10 · 175/10 pitch 4; payee 30/19; amount 148/19; words 25/27; A4 carrier); end-to-end test check (000124, ₱1M, Suyisng) created → printed (printCount 1) → void. **RECORD BLOCK (update):** sa carrier sheet (check taped sa taas), may kasamang **record print sa ibaba** ng check sa visible bond paper — CHECK RECORD title + Date Created · Bank · Check No. · Payee · Amount (PHP) · Amount Words · Due Date · Agent/Contact (mula sa supplier) + signature line "Name & Signature"; per-bank settings `record_enabled/record_x/record_y/record_font_size/record_line_mm` (default X15 Y80mm, 9pt, 5mm line) sa ⚙️ Print Position Settings; kasama sa TEST PRINT (sample agent JUAN DELA CRUZ / 09171234567). Cache-buster `v=20260910c`. **GOTCHAS:** (1) ang maliit na check (6.25×2.75") ay hindi dedetect ng HP Smart Tank (PaperOut kapag check lang ang nasa tray) — kailangan ng **A4 carrier sheet** (check taped flush sa top-left) at paper size = A4 sa template; (2) stuck HP jobs ("Printing, Retained", 0 pages) → spooler restart (`Stop-Service Spooler` → delete `C:\Windows\System32\spool\PRINTERS\*` → `Start-Service Spooler`); (3) default printer ng server = XP-80C thermal — laging i-specify ang HP Smart Tank sa template; (4) RCBC date field = i-calibrate sa UI (date_mode boxes/line).

## Latest Change (2026-09-10) — 🔧 WAWANG MOSQUITO COIL barcode change: naibalik ang stock + gumagana na ang bagong barcode sa POS

**Issue:** pagkatapos palitan ang barcode sa master catalog (WAWANG MOSQUITO COIL, id 654: `6925419077952` → `6925419077969`), "stock gone" sa dashboard at "cannot barcode in POS". **Root cause:** may DUPLICATE product row sa lahat ng store — luma `...952` (July 12, may stock: HQ 44 / HVR 55) at bago `...969` (Aug 25, 0 stock). Sa HQ/HVR INACTIVE ang bagong row kaya hindi ma-scan; ang dashboard ay nagma-match ng stock **by barcode** → `...969` (0) ang nakikita. **Fix (agent SQL sa 4 stores + cloud push via `POST /api/products`):** HQ `Id 5891` at HVR `Id 659` barcode → `...969` (napanatili ang stock 44/55 at sales history); duplicates `5944`/`713` → barcode NULL + inactive (UNIQUE barcode sa SQLite kaya kailangang i-free muna); Naic/ACGS `Id 651` (luma, 0 stock) → inactive (buhay ang `710` na may bagong barcode). Cloud `products` rows updated (NOTE: `created_at` ay NOT NULL — kailangang isama sa sync payload). **Verified:** master id 654 bc `...969`; dashboard SERVER stock = 44, TOTAL = 99; active local rows sa 4 stores = bagong barcode.

## Latest Change (2026-09-09) — 🧑‍🤝 SUPPLIERS registry + 2–3 supplier tags sa MASTER CATALOG (dashboard)

**Request:** supplier registry (company/agent/contact) + mag-link ng 2–3 supplier bawat item — para sa susunod na "repeat purchase base sa supplier" project. **DB:** tables `suppliers` (company_name/agent/contact_no/notes/is_active) + `master_product_suppliers` (product_id, supplier_id, PK both). **API:** `GET/POST /suppliers`, `PUT /suppliers/{id}`, archive/restore (soft); master list & single GET — may kasamang `suppliers[]` at `supplierIds`; `POST/PUT /products/master` — tinatanggap ang `supplierIds[]` (max 3, i-replace ang tags via `ReplaceProductSuppliers`). **Dashboard:** sidebar **POS CLIENT → Suppliers** (table + add/edit modal + archive/restore + search + show-archived); **Master Products** — Supplier column (chips 2 + "+N", hover tooltip w/ agent/contact), **All Suppliers filter** dropdown; **product editor** — "🧑‍🤝 Suppliers" section (chips w/ ✕, search-as-you-type pick hanggang 3). Cache-buster `v=20260909c`. **Verified live:** supplier CRUD (create/list/update), tag 247→supplier, untag, archive — lahat OK; served files may bagong panel. Cleanup: test supplier archived.


## Latest Change (2026-09-09) — 🖨️ SERVER-SIDE PRINT API (para sa check printing recording app)

**Context:** ang printer (HP Smart Tank 580, A4) ay naka-install sa SERVER PC; kailangan makapag-print ang gagawing "check printing recording app" mula kahit saan. **Dahil ang JumongCloudAPI ay tumatakbo sa server mismo** → dinagdag ang `PrintController` (`api/print`): `GET /printers` (listahan + default), `POST /text` {printer, text, paperWidthMm(80 thermal/210 A4), fontSize, fontName, thermal, copies} — PrintDocument na may A4 o thermal paper size, at `POST /image` {printer, imageUrl o base64 data URI} — fit-to-page A4. Security: `?key=JumongPrint2026` o header `X-Print-Key` (401 kapag mali). Package: `System.Drawing.Common 8.0.10`. Test page: `https://admin.jumongdev.com/print-test.html` (list printers → piliin → i-print). HP Smart Tank ay naging default printer sa server. **Verified live:** printers list (HP Smart Tank 580-590 series = default · XP-80C 5-25 · XP-80C · XPS · Print-to-PDF · Fax), wrong key = 401, test page 200. API redeployed (1.1.75 + PrintController).


## Latest Change (2026-09-09) — Customer Order History 🧾 modal: idinagdag ang Mode of Payment + GRAND TOTAL (kulang kanina)

**Feedback:** "may product na view sa resibo pero wala yung total payment at mode of payment". **Fix (web-only, walang API change):** sa Customers → Order History 🧾 VIEW modal — nagdagdag ng payment strip (Mode of Payment · Ref · E-Wallet Paid · Status ✅/unpaid — mula sa `/sale-items` para POS, `/shop/orders/{id}` para e-commerce, row fields para wholesale) + **GRAND TOTAL footer** (bold). `components.js` — `custMeta` state per type; `index.html` — amber info strip + total footer. Cache-buster `v=20260909a`. Deployed sa `C:\JumongAPI\wwwroot\` (components.js/index.html), verified live (custMeta + GRAND TOTAL present).


## Latest Change (2026-09-09) — RESIBO: laging may ⭐ points / 🚶 walk-in status (print, sync, dashboard Recent Sales, Customer Order History full receipt)

**Request:** "dapat malinaw sa resibo kung may earn points o walk-in lang" — kumpleto sa LAHAT: POS printed receipt, POS Reports, data papuntang PG, dashboard Recent Sales, at Customer purchase history na may **buong laman ng resibo tulad ng Recent Sales**. **POS client (1.1.79):** `Sale` + `CustomerQr`/`TotalPointsEarned` (DB migration sa startup); `SalesForm` — compute bago i-save + snapshot; `SaleService.SaveSale` insert + `MapSale`/`GetByInvoiceNo` mapping; `SyncService.SyncSale` payload + fields; **`PrinterService`** — LAGING may loyalty line: QR member → `POINTS / Previous · Earned (+0 rin) · New`; may customer walang QR → `Walang points account`; walk-in → `Walk-in — walang points` (gumagamit ng sale snapshot kahit reprint w/o customer); `ReportsForm` — bagong **POINTS** column (`⭐ +N` / `🚶 Walk-in`). **Cloud API (1.1.75):** PG migration `sales.customer_qr` + `sales.total_points_earned`; `SyncController` DTO+insert+upsert (@p19/@p20); `GET recent-sales` at `GET sale-items` — kasama ang fields; `GET customers/{id}/orders` — pos rows may `storeId`+`qr`, wholesale may `id` (saleId). **Dashboard:** Recent Sales (2 tables) — **Points column** (`pointsBadge` window helpers) + **sale modal header** (`⭐ Member: <name> · Earned +N pts` / `🚶 Walk-in / walang points account`); Customers → Order History — **🧾 VIEW** per order → modal ng **buong resibo**: POS = kapareho ng Recent Sales (Product·Qty·Pcs/Qty·Price·Total·Unit Cost·Total Cost·Pts·Profit·Margin + Revenue/Cost/Profit summary via `/sale-items`); E-commerce = items via `/shop/orders/{id}`; Wholesale = items via `/warehouse/sales/{id}/items`; note kung ⭐ +pts o walang points. Cache-buster `v=20260908b`. **Deploy:** API live (version 1.1.75; PG columns verified; recent-sales row sample qr=''/pts=0 for old sales — by design), dashboard 200; client drop (211,707,324 B) → `C:\JumongAPI\client\`. **PENDING:** GitHub release v1.1.79 + store rollout (UPDATE APP/agent) — kailangan ng GitHub token (gh hindi naka-login sa dev PC).


## Latest Change (2026-09-08) — 💵 REMITTANCE STATUS dashboard (simple UI: NASA DRIVER vs NASA TINDAHAN) + Sep-7 backlog cleared

**Request:** "PAANO MA-CHCHECK ANG NA-REMIT NA SA HINDI" + "PAKI AYOS UI YUNG MADALING MAINTINDIHAN" + confirm na ang ₱66,611 (Sep 7) ay na-turn over na. **Data fix:** ang 28 payments ng Sep 7 (₱66,611.00) na dating in-unremit para sa end-shift (na hindi natuloy — empty close 22:27 na-delete) ay minarkang `remitted=TRUE, remitted_by='Admin - na-turn over Sep 7'` → pending = 8 na lang (Sep 8, ₱29,187.50: cash 28,579.50 + gcash 608, kasama ang SHOP-20260907-0190 na na-deliver na). **Backend (`GET /remittances` — reworked):** `pending` (per driver: count/amount/cash/gcash/oldestDate/byDay), `pendingRows` (per-order unremitted), `remitted` (7-day per-order rows: orderNo/method/amount/deliveredDate/remittedBy/remittedAt + total/count), `closedShifts` (today). Read-only — walang accept. **Dashboard UI:** inalis ang lumang panel sa Online Orders; bagong **standalone sidebar item na `💵 Remittance`** (POS CLIENT) — 2 malalaking kard: 🟠 **NASA DRIVER PA** (₱, count, driver, byDay breakdown, "👉 END SHIFT sa app") vs 🟢 **NASA TINDAHAN NA** (7d total/count + closed shifts ngayon); isang listahan ng lahat ng koleksyon (Delivered · Order · Paraan · Halaga · Sino·Kailan · Status chip 🟠/🟢); filter chips LAHAT/NASA DRIVER/NASA TINDAHAN; CSV; auto-refresh 60s. `components.js` — `remittancePanel`. Cache-buster `v=20260908a`. **Driver app v2.0.8** (versionCode 10): mas malinaw na END SHIFT subtitle ("PERANG ISUSUKO MO SA TINDAHAN... kasama ang lahat ng hindi pa na-remit kahapon at ngayon"). **Deploy:** API (publish → stop/copy/start, 0 errors) · wwwroot · APK 2.0.8 signed (aapt verified) + driver-version.json. **Verified live:** pending = kenneth 8/₱29,187.50 (byDay Sep 8), remitted 7d = 78/₱141,877.05, closedShifts 0; dashboard 200; components.js may remittancePanel. **Action:** i-update ni kenneth ang app (2.0.8) → 🔚 END SHIFT → 8 payments ₱29,187.50.


## Latest Change (2026-09-07) — DASHBOARD PRICE CHECK panel (mga presyo na kulang sa 5% markup)

**Request:** "pwede paki bigyan ako viewing in dashboard" para sa price check (mga produkto na hindi umaabot sa 5% markup). **Backend:** bagong `GET /dashboard/price-check` (DashboardController) — master_products default-unit basis: `price` (default unit price) vs `cost` (unit cost = unit.cost o base×qpu), markup%, tag (FIX / BOX/PROMO — by-N/case/buy-10 heuristic o qpu≥5 / TOBACCO), online_price + online markup kung >0; filters `maxPct` (default 5), `tag`, `category`; summary counts + categories. **Frontend:** sidebar POS CLIENT → `📉 Price Check`; section panel (index.html) — filters (tag/category/threshold), summary chips (below cost/total/FIX/Box/Tobacco), table: Name·Barcode·Category·Unit·Price·Cost·Markup% (kulay: red<0, amber<3)·**Suggested (cost×1.05 round-up ₱0.25)**·Online price·Tag; sticky header + scroll; **CSV export** (Blob, BOM para Excel). `components.js` — `priceCheckPanel` Alpine component. Cache-buster `v=20260907b`. **Verified live:** /price-check → 273 items (belowCost 1 · FIX 220 · BOX/PROMO 43 · TOBACCO 10 · 31 categories), dashboard/components.js HTTP 200. Ang pag-aayos ng presyo ay MANUAL pa rin sa Master Products (gaya ng napagpasyahan) → SYNC FROM CLOUD sa stores.


## Latest Change (2026-09-07) — DRIVER END-SHIFT CRASH FIXED (v2.0.7) + dashboard collection REMOVED (remittance → driver app lang)

**Issue:** "check driver app when clicking the endshift the closes" — nag-crash ang app sa tuwing binubuksan ang END SHIFT. **Root cause (nakuha via bagong crash-report):** `renderCarryOver` NPE — `findViewById(esCarryEmpty)` ay tinawag PAGKATAPOS ng `box.removeAllViews()` (naalis na sa tree ang child) → null → crash. Sa 2.0.5 (no guard): app closes; sa 2.0.6 (crash-guard): na-caught + na-report (crash id 245, device 23053RN02A). **Fix (2.0.7):** huwag i-remove ang `esCarryEmpty` (child #0) — tanggalin lang ang mga lumang ROW (`while childCount>1 removeViewAt(last)`), i-toggle lang ang visibility. **Dashboard collection REMOVED (owner):** "paki alis na yung sa dashboard collection — ilipat na lahat sa mobile driver app ang endshift" → `POST /payments/{id}/remit` = **403** (remittance = driver app END SHIFT, 1x/day); `GET /remittances` = `{closedShifts}` lang (walang pending list/open shifts); REMITTANCE panel sa index.html = info lang ("Driver Remittance — End Shift" + closed shifts ngayon); components.js — tinanggal ang `remitPayment` + payments state. Legacy 27 'open' driver_shifts rows: hindi na ipinapakita. **Deploy:** native 2.0.7 (versionCode 9, signed, aapt verified) → `wwwroot/updates/JumongDriver.apk` + `driver-version.json` 2.0.7; API redeployed (stop→copy→start, 0 errors, dll 21:54). **Live verified:** GET /remittances `{"closedShifts":[]}` · POST /payments/1/remit → **403** · driver-version 2.0.7. **Estado ng pera (21:37):** 90 payments remitted (₱155,739.80, lahat by Admin — kasama ang buong batch 21:36-21:37 na nag-settle pati ngayong araw); 0 pending; order SHOP-20260907-0190 = pending ₱6,344 (wala pang payment). **Action:** i-UPDATE ng driver sa 2.0.7 → END SHIFT gagana na (walang carry-over = ok). NOTE: kung gusto nilang iwan talagang unpaid ang ngayong araw (6 payments ng Sep-7 delivered: 0171/0175/0186-0189) para i-end-shift ni kenneth — kailangan manual revert (offer pending).


## Latest Change (2026-09-07) — DRIVER APP END-SHIFT (1x/day) + REMITTANCE SA APP + carry-over (native v2.0.5, API + dashboard)

**Request (owner):** e-commerce COD cash ay nasa DRIVER (hindi sa HQ cashier — pagtanggap/order prep lang sila); gusto ng driver end-shift isang beses lang kada araw — ang hindi pa nabayaran/na-deliver ay dadalhin bukas (items nasa driver pa), may option ang driver na i-CANCEL ang hindi tinanggap (stock babalik sa HQ inventory — umiiral na ang cancel/restock flow). **Server (DashboardController):** (1) driver order list fix — isama ang `'arrived'` (`status IN shipped/arrived/delivered`) para hindi mawala sa listahan ang undelivered; (2) `GET driver/endshift-info` (auth driver token): `closedToday` + `lastClosed` (1x/day guard), `sweep` (LAHAT ng unremitted payments ng driver — cash/gcash), `today` (delivered today subset), `carryOver` (unpaid undelivered shipped/arrived w/ days-old); (3) `POST driver/end-shift`: 409 kung may closed shift na ngayon; i-sweep ang lahat ng unremitted payments → `remitted=TRUE, remitted_at, remitted_by=<driver full name>`; isara ang `driver_shifts` (closed, ended_at, totals) — pagkatapos mag-end-shift pwede pa ring mag-deliver/collect (bukas na papasok, auto sa susunod na araw); (4) `GET remittances` + `closedShifts` ngayon. **Dashboard:** REMITTANCE panel — "🔚 End shift ngayon" block (driver, orders, ₱cash/₱gcash, time). **Driver app NATIVE (DriverApp/, v2.0.4→2.0.5, versionCode 7):** pinagpalit ang 🏠 HQ → full-width `🔚 END SHIFT & REMIT` button sa main (nagiging "✅ END SHIFT DONE (bukas na ulit)" pag tapos); END SHIFT overlay: collectible count/cash/gcash cards, delivered-today line, carry-over list (DELIVER → order detail; CANCEL → existing cancel modal w/ reason), confirm dialog → POST. Build via server (dev PC walang SDK/JDK): synced sources sa server clone, `gradle :app:assembleRelease`, signed `jumong_sign.keystore` (alias jumong/jumong2026, apksigner build-tools 37.0.0) → `wwwroot/updates/JumongDriver.apk` (2,616,461 B) + `driver-version.json` 2.0.5. Legacy web driver-app.html/driver.html (v1.1.51) updated rin (web fallback). API redeployed (clean: stop → copy → start). **Verified live:** endship-info via temp token (cleaned up) — driver kenneth (16): sweep 21×₱45,110 cash (19 delivered ngayon = ₱43,821; 1 payment ₱288 na-ACCEPT ng admin mid-test — gumagana ang dashboard), carryOver 0; updates URLs HTTP 200. **Action sa store:** i-UPDATE ng driver ang app (2.0.5 in-app) → tapat na END SHIFT bago isuko ang ₱45k+ cash → dashboard REMITTANCE = 0 pending + closed shift row. NOTE: 27 stale 'open' driver_shifts (lumang return-to-hq rows) ang naka-display sa dashboard open-shifts list — legacy, hindi na ginagamit ng bagong flow.


## Latest Change (2026-09-06) — Dashboard DEDUCT STOCK chip Alpine fix (prod-null errors) + favicon; transient tunnel blip ruled out

**Reported:** console errors sa admin.jumongdev.com — 6× `Alpine Expression Error: Cannot read properties of null (reading 'name'/'barcode'/'price'/'stockParentId'/'stockParentName'/'linkRatio')` + 6× uncaught TypeErrors sa bawat page load; plus `/products/master` 400 ×2 at ERR_CONNECTION_CLOSED sa `warehouse/transfers/pending-count` at `shop/orders/new-count` polls. **Root cause (frontend bug):** ang DEDUCT STOCK product chip (`index.html`) ay nasa `x-show="prod"` — pero ang `x-show` ay HINDI pumipigil sa Alpine na i-evaluate ang mga child bindings (nasa DOM pa rin, display:none lang) → kapag `prod` = null (initial state, `deductPanel.prod: null` sa components.js) sumasabog ang 6 na expression sa TUWING mag-load ang dashboard (kahit hindi binubuksan ang DEDUCT section). **Fix:** pinalitan ng `<template x-if="prod">` wrapper (hindi na nire-render ang children habang null; `x-show` guard pattern na may children na direktang nagba-bind = BUG sa Alpine — gamitin ang x-if sa ganitong kaso). Plus: `<link rel="icon">` inline SVG data-URI (walang `/favicon.ico` 404). **Server-side = HINDI issue (transient lang):** live tests 200 lahat; API service uptime 23h (walang restart — start 09-05 17:50), PG up since 08-18, walang .NET/ASP.NET exceptions sa Application log; ang error timestamps (16:54 / 17:14-17:15 PHT) = maikling Cloudflare tunnel blip lang (ang 30s badge polls + concurrent fetches lang ang naapektuhan; self-heal via try/catch). Deployed: isang file lang (`wwwroot/index.html`) → server `C:\JumongAPI\wwwroot\` (walang build/restart), verified live HTTP 200 (`x-if` present, favicon present). Ang Tailwind CDN production warning = known/accepted (nangangailangan ng build pipeline — hindi worth it sa internal dashboard).

## Latest Change (2026-09-05) — v1.1.78 REMOTE ROLLOUT: lahat ng 4 na stores na-update via agents (exe swap 201MB each: download→kill→swap→restart)

**Paraan:** agent `update` command (GitHub v1.1.78 exe → `JumongPosV1.01.exe.new`, verified 211,690,940 B) + agent `ps` (Stop-Process → Copy → Remove → Start-Process). Paths: HQ `C:\Users\ADMIN\Desktop\JumongPosHW\` · HVR `C:\Users\ADMIN\Desktop\HVR_POS\` · Naic/ACGS `C:\JumongPos\`. Lahat ng 4 ay **1.1.78, outdated=False** (agent/status verified). **Naic quirk:** ang agent nito ay lumang build (hindi na-deploy ang 2026-08-15 per-heartbeat refresh fix) → nag-cache ng appVersion=1.1.70 kahit 1.1.78 na ang exe/DB → **na-restart ang agent** (restart_agent.cmd: taskkill → start `C:\JumongPos\agent\Agent.exe`) → 1.1.78 na ang ulat. Naic err=True (HttpRequestException sa sync — internet wobble, hindi blocking). Ang transfer stock-safety fix ay live na sa lahat ng stores.

## Latest Change (2026-09-05) — v1.1.78 STOCK-SAFETY: transfer receive ay HINDI na nag-local-receive ng SHORTAGE items (Redhorse #1007 phantom fix) + Redhorse data repair

**Issue:** "yung redhorse 500 bakit pumasok nanaman mali sa acgs ngayon yung 72" — 2nd occurrence ng parehong bug (Nescafe #981). **Timeline:** 15:53 HQ +65 (brew RECV-20260905-155302, 7→72) → 15:54 transfer **#1007** (ACGS, Redhorse Extra Strong 500ml by 12, qty 72) → 15:56 transfer #1009 (HVR) na-receive muna (HQ −5 → 67) → 16:07 ACGS receive #1007: server **rejected** (HQ 67 < 72, received_qty=0, status partial) ngunit **ACGS POS local ay nag-receive pa rin ng +72** (2→74) → pagkatapos: totoong transfers #1013/#1008 (Naic +10 each) at #1014 (ACGS +20). **Kasalukuyang estado ngayon:** HQ 25 · HVR 5 · Naic 22 · ACGS 93 (= 2 base + 72 phantom + 20 totoo − 1 benta). **Code fix (PendingOrdersForm.cs):** pagkatapos ng server response, i-filter ang `result.Shortages` — ang **lokal na pag-receive + stock snapshot ay para LANG sa mga item na na-accept ng server** (shortageIds hashset); malinaw na UI message: `⚠ HINDI na-receive (kulang ang stock sa HQ): <items> — mananatiling PARTIAL` (dati: nag-receive pa rin + tahimik na "N item(s) shortage"). **Data repair:** ACGS local 93→**21** (−72 trail `Transfer #1007 phantom +72 undo (server shortage - kulang ang HQ stock)`, Synced 0→1) — verified cloud: HQ 25/HVR 5/Naic 22/ACGS **21**. Release: AppVersion 1.1.78 · latestVer 1.1.78 · exe (211,690,940 B, sha256 ece56dea…) → drop + **GitHub release v1.1.78** (download 200) · API redeployed. **Stores: UPDATE APP (lalo na ACGS/Naic — sila ang nagre-receive ng transfers).** Lesson: kung ang isang item ay nasa server response Shortages — HINDI ito dapat i-local-receive (kailangan din ng 'partial' re-receive flow sa hinaharap: kapag may stock na ang HQ, gumawa ng bagong transfer).

## Latest Change (2026-09-05) — BIRCH by-8 box ratio correction (master 730: 160→20) + stock data repair sa HQ/Naic/ACGS

**Issue:** "check the birch i received lastnight thru mobile i received 20 boxes why converted to pcs?" — ang mobile receiving noong Sep 4 14:49 ng "20 boxes" ay naging **+3200** sa HQ Birch by-8 (6→3206). **Root cause:** bagong master **730 "BIRCH TREE MILK FORTIFIED MILK 20/8/33G"** (nilikha 14:43, audit ×3) ay naka-link sa 51 (by-8) na may **link_ratio 160** (ang user ay nag-isip sa sachets: 20 sleeves × 8 = 160) — ngunit pagkatapos ng by-8 conversion (14:32) ang stock base ng 51 ay nasa **PACK units na**, kaya ang tamang ratio ay **20** (1 box = 20 by-8 sleeves) → ang +3200 ay **8× ang sobra** (dapat +400). Ang mga transfer #1000 (Naic 320) at #1001 (ACGS 320) noong Sep 5 07:22/07:26 ay galing sa maling 3205 na base → mali rin. **User fix:** in-update ang 730 ratio 160→**20** (17:35, master_product_audit). **Data repair (lokal per store, Sep 4 pattern, Synced 0→1 auto-push):** HQ 2562→**402** (−2160 trail: 6 base + 20 boxes×20 = 406 − 4 benta) · Naic 343→**23** (−320: 25 base − 2 benta) · ACGS 333→**13** (−320: 13 base) · HVR 29 (hindi naapektuhan). Reference: `Birch 730 ratio correction 160->20: undo +3200 at transfers #1000/#1001` (User System). Cloud verified (17:39): 402/23/13/29. Lesson: kapag gumagawa ng linked child (box) matapos ang pack conversion, ang ratio ay dapat nasa PACK units (hal. 20), HINDI sachets (160).

## Latest Change (2026-09-05) — HQ POS Customer Chat v1.1.77: message bubble rendering fix (hindi nababasa ang mensahe)

**Bug:** "that chat u make in pos client does cannot see messages properly" (HQ ay naka-1.1.76 na, walang crash sa error.log). **Root cause (CustomerChatForm.MakeBubble):** ang text Label ay `AutoSize = false` — sa WinForms ang Label ay **HINDI nag-wrap** kapag naka-off ang AutoSize → ang mahahabang mensahe ay **pinuputol** (isang linya lang / clipping) at ang label ay may default na background (hindi tugma sa bubble fill) → hindi maayos na nababasa. **Fix:** ginamit ang tamang WinForms pattern — `AutoSize = true` + `MaximumSize = new Size(maxTextW, 0)` (wrapping auto-height label), kinukuwenta ang sukat sa pamamagitan ng `GetPreferredSize` (ginagamit din para sa bubble dimensions), at `BackColor` ng labels = bubble fill color. Build 0 errors. Release: AppVersion → 1.1.77 · latestVer → 1.1.77 · exe (211,690,940 B, sha256 6ba10ff4…) → drop + **GitHub release v1.1.77** (id 383186840, download HTTP 200) · API redeployed. HQ: UPDATE APP ulit → i-test ang chat messages (long messages, multi-line).

## Latest Change (2026-09-05) — HQ POS CUSTOMER CHAT (cashier sumasagot sa e-commerce chat) + customer contact card (client v1.1.76 + API 1.1.74)

**Request:** "pwede ba exclusive sa HQ pos client mag ka roon ng messenger so the cashier can answer the message from the ecommerce either the admin in dashboard or the cashier on duty to reply so if ever i am not in the dev pc the cashier can answer" + "pag customer message ilagay ang contact number sa ibaba ng kanyang card at subdivision para madaling malaman kung saan at ano ang kanyang contact". **Decisions:** (1) HINDI ipinapakita ang pangalan ng cashier sa customer (Andengs Superstore lang — naka-save sa history ang cashier name); (2) CHAT button sa customer bar (itaas); (3) default address ang ipapakita. **Server (API 1.1.74):** migration `shop_chat_messages.reply_by TEXT` (ADD COLUMN IF NOT EXISTS); `GET /chats` enriched — LEFT JOIN customers (phone) + LATERAL default address (block/lot/subdivision/address) mula sa customer_addresses; `GET /chats/{id}/messages` → may reply_by; `POST /chats/{id}/reply` → tumatanggap ng `replyBy`. **Dashboard (web):** conv list item at thread header ay may `📞 phone · 🏘️ subdivision` line; admin bubbles ay nagpapakita ng `replyBy` (cashier name) para malaman mo kung sino ang sumagot; nagse-send ng replyBy = web username. Cache-buster v=20260917. **POS client (v1.1.76):** `Models/CloudChat.cs` (payload DTOs); `SyncService` chat helpers (GetChatConversations/GetChatMessages/GetChatUnread/ReplyChat/MarkChatSeen — light, walang sync queue); bago `Forms/CustomerChatForm.cs` (HQ messenger: conv cards na may pangalan + 📞 phone + 🏘️ subdivision (default) + unread badge + last message/time; thread na may chat bubbles (customer violet/ikaw gray, rounded), reply box Enter-to-send, auto-poll 5s, auto mark-seen; cashier name ang naka-save sa reply_by); `SalesForm` — `💬 CHAT` button sa customer bar (dock right, teal) na may pulang unread badge (poll 10s via timer, HQ lang; `--devchat` command-line override para sa dev testing). **Verified end-to-end (API):** test customer (na may address row) → send message → GET /chats enriched `phone=09171234567 subdiv=Pasinaya North block=9 lot=7` ✓ → reply `replyBy='Cashier Test'` → messages may reply_by ✓ → customer GET sees reply (unreadCustomer=1) ✓ → full cleanup (0 rows). Release: AppVersion 1.1.76 · latestVer 1.1.76 · exe 211,690,940 B (sha256 850f2631…) → drop · **GitHub release v1.1.76** (id 383157435, download HTTP 200) · API redeployed (v1.1.74, reply_by column exists). Stores UPDATE APP — **HQ lang ang may CHAT button**. NOTE: ang CustomerChatForm visual ay para i-verify sa HQ (o dev --devchat).

## Latest Change (2026-09-05) — CHECKOUT modal "nag-appear lang at close agad" FIXED: overlay/history race sa cart drawer → checkout (shop.html)

**Sintomas (pagkatapos ng TDZ fix):** "nag-oorder na daw sila tapos pag click check out nag-appear lang at close din agad" — gumagana ang add-to-cart, pero kapag nag-click ng CHECKOUT **sa loob ng cart drawer**, bumubukas at agad nagsasara ang checkout modal. **Root cause (race):** `openCart()` → `history.pushState({o:'cartDrawer'})`; i-click ang CHECKOUT → `openCheckout()` → `closeCart()` → `popOverlay()` → **`history.back()`** (async) → agad na nag-show ng checkoutModal + `pushOverlay()` → `pushState({o:'checkoutModal'})` → ang **stale popstate** mula sa `history.back()` ay dumating → ang popstate handler (DOM-top based) ay nakita ang top = 'checkoutModal' → **`closeCheckout()` agad**. (Hindi nahuli ng unang E2E dahil diretso ang tawag — hindi dumaan sa cart drawer.) **Fix:** `openCheckout()` → gawing **SWAP nang walang history.back()**: i-hide ang cart drawer/overlay nang direkta + i-`splice` ang 'cartDrawer' sa `overlayStack` + ipakita ang checkoutModal + `pushOverlay()` — walang back → walang stale popstate → hindi na agad nagsasara. **Verified (CDP, real customer path, member session sa shop.jumongdev.com):** BAGO ang fix: `modalState: HIDDEN (nagsara!)` (reproduced); PAGKATAPOS: **3/3 runs `VISIBLE (buo)` + mga order na SHOP-20260905-0133/0134/0135 na nagtagumpay** (cancelled at na-cleanup lahat — customers/sessions/orders/trails = 0). Deployed → **LIVE** (production HTTP 200, may SWAP marker). Ang checkout ay fully working na: guest 0 JS errors + member drawer→checkout→place order end-to-end.

## Latest Change (2026-09-05) — 🔥 E-COMMERCE CHECKOUT FIXED (walang order mula Sep 3 16:00): TDZ script-kill sa shop.html boot — root cause at verified end-to-end

**Sintomas:** "sabi ng mga customer d sila maka check out... kahapon wala order, dyos ko" — huling online order: **SHOP-20260903-0129 (Sep 3 16:00)**, mula noon zero orders (walang kahit cancelled). **Investigation:** API/DB/disk OK (disk 39GB free; ang mga 53100/10054 sa api_error.log ay lumang startup entries mula noong unang beses na na-migrate ang sale_items UNIQUE constraint — hindi ang dahilan). Frontend static audit malinis → **headless Chrome CDP smoke test** ang nakahuli: `ReferenceError: Cannot access 'feedLoading' before initialization` sa bawat page load. **ROOT CAUSE:** ang top-level BOOT block (`loadBranding(); loadPromo(); loadCatalog(); ... loadMe(); initFeed(); switchView(...);` etc.) ay nasa **GITNA ng pangunahing script** (dating line 1733-1746) — ngunit ang mga `let feedPosts/feedLoading/feedDone` (dating 1810), `let profileOpen` (1968), `const CAT_ICONS` (1947) at iba pa ay **declared SA IBABA ng boot** → ang `initFeed()` (1741) ay nag-access ng `feedLoading` sa **TDZ** → ReferenceError → **ang buong script execution ay humihinto sa 1741** → ang LAHAT ng top-level code sa ibaba ay hindi kailanman na-execute (feed patay, `profileOpen` sa TDZ forever) → **ang member login flow ay nasira rin** (`loadMe` → `if (profileOpen)` → ReferenceError sa loob ng async loadMe bago ang applyMemberUI) → ang mga logged-in customers ay hindi na-apply ang member UI (walang presyo, walang add-to-cart) → **walang makapag-order**. Na-introduce noong Sep 3 gabi (FEED fixes/cleanup release na naglipat ng feed code sa ibaba ng boot nang hindi napansin). **Fix (shop.html):** inilipat ang buong boot block **sa DULO ng script #1** (pagkatapos ng lahat ng let/const declarations; bago `</script>`) — walang TDZ. Plus: na-define ang kulang na `function showBubble()` (chat bubble ng bagong in-app chat). **Verified:** (1) guest CDP load = **0 JS exceptions**, feed gumagana; (2) full MEMBER end-to-end via CDP (test customer + customer_sessions token sa shop.jumongdev.com): member=true → add to cart (555 TUNA ADOBO ₱29.75) → checkout modal → address filled → **PLACE ORDER SUCCESS → SHOP-20260905-0132 created** → cancelled via admin API (hold released, trail −1/+1) → full cleanup (order/customer/session/trails deleted, verified 0). Deployed sa server wwwroot → **LIVE** (verified: boot marker + showBubble present sa https://shop.jumongdev.com/shop.html HTTP 200). Service worker ay network-first → mag-refresh ang customers sa reload. NOTE: Na-deploy muna ang checkout fix bago ang chat-feature side fixes (nasa itaas na entry ang chat; magkasama silang live).

## Latest Change (2026-09-05) — E-COMMERCE IN-APP LIVE CHAT (customer ↔ admin): real store staff replies via dashboard (API 1.1.73)

**Request:** "ibalik ang messenger sa apps para live nakaka-chat ang customer to admin sa dashboard... admin reply thru dashboard, customer message from ecommerce app" + "wag na ai real person reply" — WALANG AI: ang customer ay nagme-message mula sa shop app; ang ADMIN (may-ari) ang sasagot sa dashboard nang live. **DB (PgDatabaseHelper):** `shop_chat_conversations` (customer_id UNIQUE, name, last_message/last_sender/last_at) + `shop_chat_messages` (sender customer/admin, seen_by_admin, seen_by_customer) + indexes. **API (DashboardController, version → 1.1.73):** customer side — `GET shop/chat` (member session via jshop cookie; conv+messages+unreadCustomer; 401 kung walang session), `POST shop/chat` (send, max 1000), `POST shop/chat/seen`; admin side — `GET chats` (conv list + unread), `GET chats/unread` (global badge), `GET chats/{id}/messages`, `POST chats/{id}/seen`, `POST chats/{id}/reply`. **shop.html:** floating 💬 bubble (right, above bottom nav) + chat sheet (guest → SIGN IN CTA; member → thread + composer); poll 4s; unread red badge sa bubble; auto mark-seen kapag bukas ang sheet; walang HTML injection (textContent). **Dashboard:** E-COMMERCE group → `💬 Customer Chat` child item (naka-map sa groupParents; pulang badge sa sidebar kapag may unread customer message — poll 30s sa bottom script after login) + `chatsPanel` component (components.js): two-pane (conversations list + thread), poll 4s habang bukas, auto-seen, Enter-to-send. Cache-buster v=20260916. **Verified live:** API v1.1.73 (tables created); chats/unread 0; shop/chat w/o session = 401; end-to-end smoke test (insert TEST conv → GET /chats unread 2 → messages → seen → unread 0 → reply ok) → test data deleted (0 convs); served files HTTP 200 (index/shop/components). node checks OK. Walang client release — API + web lang. NOTE (pending pa): checkout root-cause investigation (walang order since Sep 3 — ipinagpalit sa chat feature na ito; hindi pa tapos) + MONITOR dashboard feature.

## Latest Change (2026-09-05) — POS client v1.1.75 HOTFIX: customer picker crash (SelectCustomerForm NRE) — dahil sa bug ng v1.1.74

**Bug (v1.1.74):** "click ko customer wala naman lumabas... pati scan customer d naman din nagana" — ang initial customer-list load (`RefreshGrid("")`) ay nasa **CONSTRUCTOR** ng SelectCustomerForm → `dgvCustomers.DataSource` bind habang **wala pang window handle** → `NullReferenceException` sa `DataGridViewBand.set_Thickness` (crash log 10:25/10:27 sa dev; `SelectCustomerForm..ctor line 17` → `SalesForm.AttachCustomer line 144`). Epekto: ang pag-click ng 👤 CUSTOMER ay pumapatay sa POS nang tahimik (ThreadException handler logs + exit) — pati ang 📱 SCAN QR ay "hindi gumagana" dahil patay na ang app. **HQ at HVR ay na-install na ang buggy 1.1.74** (10:xx update) → kailangan ng bagong release (string-compare ng UpdateService: 1.1.74 client vs 1.1.74 release = walang update prompt). **Fix (v1.1.75):** `SelectCustomerForm.cs` — inilipat ang `RefreshGrid("")` mula sa ctor papunta sa **`Shown` event** (may handle na; parehong pattern ng ProductSearchForm). `SalesForm.cs` — try/catch + MessageBox sa `AttachCustomer()` at `ScanCustomerQr()` (hindi na tahimik na namamatay ang app kung may ibang error; naka-log sa error.log). Build 0 errors. Deployed: AppVersion → 1.1.75 · API latestVer → 1.1.75 + API redeploy (stop→copy→start ✓; agent status lahat outdated=True) · exe 211,559,868 B (sha256 a6a4fdd4…) → server drop · **GitHub release v1.1.75** (id 383112330, draft→asset→publish, download HTTP 200). Dev PC: v1.1.75 bukas para sa test (user na mismo ang magvi-verify ng CUSTOMER/SCAN QR). Stores: UPDATE APP — HQ/HVR (mula buggy 1.1.74), ACGS (1.1.73), **Naic pa rin 1.1.70 (hiwalay i-verify kung bakit laging huli)**. GOTCHA (re-learned): huwag mag-DGV DataSource bind sa ctor — sa Shown/Load pagkatapos ng handle creation.

## Latest Change (2026-09-05) — POS client v1.1.74: CUSTOMER FLOW REWORK — tinanggal ang Order Type modal; default WALK-IN top; customer attach anytime (habang nagpu-punch)

**Request:** "dba every after the transaction ordertype appear? pwede palitan... remove ang counter... track customer at new session nalang... appear yung customer selection pag walang customer name asa top yung default na walk-in na d nag earn ng points... pag nasa loob na sya na transaction biglang sabihin ng customer i have id so my option sa loob ng cart to select customer while punching? possible ba yun?" — ang lumang UX ay may **OrderTypeForm modal** ([WALK-IN] / [COUNTER (Track Customer)]) na lumalabas sa **startup** (SalesForm.OnLoad) at **pagkatapos ng bawat transaksyon** (SalesForm.cs:926). **Desisyon ng user (3 tanong):** (1) customer picker = TOP row `🧾 WALK-IN (no points)` para ma-detach; (2) lahat ng may customer = OrderType **Counter** sa ulat (dati ang QR-scan ay 'Walk-in'); (3) customer buttons sa **customer bar sa itaas** (laging visible). **Changes:** `SalesForm.cs` — tinanggal ang mga PromptOrderType/PromptNextTransaction calls (OnLoad + post-sale = auto new session, walang prompt); bagong `AttachCustomer()`; `UpdateCustomerDisplay()`: walang customer → `🧾 WALK-IN — walang points` + gray WALK-IN chip (laging visible); may customer → `CUSTOMER` chip; bagong `👤 CUSTOMER` button (violet, docked right katabi ng 📱 SCAN QR na **laging visible na**) + clickable info bar; `ScanCustomerQr` → `_orderType = "Counter"`. `SelectCustomerForm.cs` — `RefreshGrid` sa open kahit walang search (GetAll active, sorted) + prepend ang WALK-IN row (Id 0 → SelectedCustomer=null = detach); SelectCurrent lookup via `_list` (hindi na re-Search). **Inalis:** `Forms/OrderTypeForm.cs` (deleted — tanging refs ay ang mga inalis na calls). Held-cart restore at PaymentForm credit (customer-based, walang orderType dependency) = walang pagbabago. Build: 0 errors. Release: AppVersion → **1.1.74**; API `latestVer` → 1.1.74 (DashboardController.cs:9609) + API redeployed (stop→copy→start ✓); exe (211,555,772 B, sha256 2ec72f6d…) → server drop; **GitHub release v1.1.74** (id 382941883, draft→asset→publish, download HTTP 200). Agent status: lahat outdated=True (HQ/HVR/ACGS 1.1.73, **Naic 1.1.70 pa**). Stores: UPDATE APP.

## Latest Change (2026-09-04) — DEDUCT STOCK v2 (dashboard): current stock chips per store + 🕘 HISTORY ng adjustments (API 1.1.72)

**Requests:** "sana nakikita yung current stock nya?" + "at sana my history ako ng mga edited by admin". **Dagdag sa ➖ DEDUCT STOCK tool:** (1) **📦 current stock per store** — kapag pumili ka ng product, parallel agent queries sa lahat ng online stores (excl. STORE-DEV-0001) ang kukuha ng live stock na lalabas sa bawat store row (green/red chip; linked child = available `floor(parent ÷ ratio)` na may tooltip; 'wala dito' kung wala ang product; refreshed after bawat run). (2) **🕘 HISTORY panel** sa ibaba ng deduct form: listahan ng lahat ng adjustments mula sa **cloud `stock_trails`** (`Reference LIKE 'Adjustment:%'` — kasama ang ginawa sa POS Admin Stock Adjustment at sa dashboard tool; hindi kasama ang mga conversion refs): kailan · store · product · qty (red −N) · before→after · remark · by (UserName). Filter by store dropdown + ⟳ refresh. **New API endpoint `GET /adjust-log`** (DashboardController, ±limit 100, optional storeId — SELECT ... WHERE reference LIKE 'Adjustment:%' ORDER BY id DESC). API version → **1.1.72** (2 places: GetVersion + health). Files: `DashboardController.cs` (+18), `components.js` (deductPanel: stock chips state/`loadStocks`/`queryStock`/`stockClsOf`/`stockTxt`/`stockTip`; history state/`loadHistory`/`histDate`/`histRemark`/`histQty`; `stores()` excl. dev; run() → auto-refresh stocks+history), `index.html` (store-row stock chip + loading hints, HISTORY table block, cache-buster **v=20260914**). node --check OK · div diff 0 · API redeployed (stop→copy→start; first attempt nag-copy habang RUNNING pa → file-lock errors, inulit nang naka-stop ✓) · verified live: version 1.1.72, adjust-log 5 rows (kasama KNORR CUBES BEEF −73 'Manual adjustment' na POS-side ng user 19:10 at Mismo corrections 09-02), wwwroot HTTP 200. NOTE: history lag = hanggang mag-sync ang POS app (sarado ang app → mabagal ang cloud).

## Latest Change (2026-09-04) — Dashboard "➖ DEDUCT STOCK" tool (per-store via agents, may remark + StockTrail) — web only

**Request:** "give me power to deduct stock in all pos using agents... put the remark" → clarification: "gusto ko mag adjust ng stock PER STORE hindi sabay sabay" (ang sabay-sabay ÷8 ay conversion lang) + "wala ako access from here [sa store machine] i need to go in store" — ang umiiral na Stock Adjustment ay nasa POS terminal per-store; gusto niyang gawin remotely sa dashboard. **Feature:** bagong sidebar item sa POS CLIENT group (`deduct`, katabi ng Master Products): product search (master catalog name/barcode, walang images) → qty → **REMARK (required)** → store checkboxes (**default WALANG naka-check**; offline agent = disabled) → RUN + confirm dialog. Bawat napiling store (sequential agent `sql`, poll 20×1.5s): hanapin ang local product by barcode (fallback exact name) → **linked child = bawas sa PARENT × ratio** (gaya ng StockService) → **kung before−qty < 0: SKIP** ("kulang ang stock") — walang negative (gaya ng POS StockMovementForm.cs:281) → `UPDATE StockQty` + `INSERT StockTrail` (`Reference = 'Adjustment: <remark>'`, UserId 0, UserName 'Dashboard', Synced 0 → POS auto-push sa cloud). Result table per store: ✅ before→after / ⏭ skip / ⚠ error. `storeTrailPanel.trailType` + `typeCls`: bagong **Adjustment** badge (violet) kapag Reference nagsisimula sa 'Adjustment' (dati '—'). Files: `index.html` (sidebar + section + cache-buster **v=20260913**) + `components.js` (`deductPanel` ~1666). node --check OK · div-balance 0 diff · deployed sa server wwwroot (components 127,392 B / index 317,825 B) HTTP 200. Walang API change, walang client release. Usage: dashboard → DEDUCT STOCK → pumili ng product/qty/remark/store → lalabas ang trail sa POS Stock Movement (TYPE=Adjustment) at sa cloud trails. NOTE: nag-live ito 18:55 (gabi, off-peak) per user.

## Latest Change (2026-09-04) — BIRCH TREE MILK FORTIFIED 33G by-8 conversion (master id 51): stocks ÷8 per store (MILO/BEAR template)

**Request:** "BIRCH TREE MILK FORTIFIED 33G by 8 (748485401492) ... 615 48 remaining to by 8 can you do that for me" — master 51 ay nasa final state na (user na ang nag-rename ng "by 8" + ₱75/₱71.50 + units cleared — update 14:17, na-pull na ng lahat 14:25-26); ang stocks ay pieces pa kaya hinati ÷8: **HQ 48→6** (local 5249, trail 103408) · **AA36 256→32** (49, 65301) · **E174 200→25** (53, 20674) · **A80C 111→13** (53, 29828 — floor, remainder 7 loose sachets in-adjusted kasama sa conversion, gaya ng Kopiko Blanca decision). Bawat store: agent `UPDATE Products SET StockQty=StockQty÷8` + conversion `StockTrail` (ref `BIRCH TREE MILK FORTIFIED 33G by-8 conversion 2026-09-04`, User 'System', Synced 0→1 agad — running ang POS apps). Cloud verified: mirror 6/32/25/13 (total 76 packs) + 4 stock_trails 14:32. Resulta: 1 punch = ₱75 −1 pack; e-com ₱75/pack reserve 1. ACGS physical 7 loose sachets = hindi na-account (unitless na ang barcode).

## Latest Change (2026-09-04) — Dashboard master catalog: TOTAL STOCK cell breakdown (tooltip + "sa ibang store lang" tag kapag HQ = 0)

**Issue:** "KNORR CUBES CHICKEN SINGLES 10G — bakit may stock (306) sa list view pero pag i-check zero?" — ang master grid ay may DALAWANG kolum: **TOTAL STOCK** (sum ng parehong barcode sa LAHAT ng stores) at **SERVER STOCK** (HQ lang). Ang 306 ay galing sa HVR (AA36) na may 306 pcs noon; HQ = 0 na mula pa 2026-08-19 (INV-7159-20260819-0103) at in-zero rin ang HVR ngayong 11:28 (INV-AA36-20260904-0038, −306, channel other) — kaya lahat 0 na ngayon. Hindi bug sa computation — semantic confusion sa dalawang kolum. **UI improvement (web only):** `components.js` — `loadTotals()` ay gumagawa na rin ng `storeQtyByBc` per-store map (kasama sa cache); bagong helpers `_stockBarcode()` (parent-aware para sa linked children), `stockBreakdown(p)` (per-store listahan "HVR: 306 · Naic: 12") at `stockTitle(p)` (tooltip: "TOTAL = stock sa LAHAT ng store · SERVER = HQ store lang" + breakdown). `index.html` — TOTAL STOCK cell: hover tooltip na may breakdown, at kapag `totalStock > 0` pero `hqStock === 0` ay may maliit na amber tag na "sa ibang store lang" (para hindi na malito na stock daw pero wala sa HQ). Cache-buster → v=20260912. Deployed: components.js (120,345 B) + index.html (309,973 B) → server `C:\JumongAPI\wwwroot\` (walang build/restart), verified HTTP 200. node --check OK.

## Latest Change (2026-09-04) — POS client v1.1.73: linked child products show DERIVED stock (parent ÷ ratio) sa search popup, Products list, stats/filters

**Request:** "dba yung sa master catalog ginebra gin parent tapos yung child nakikita sa stock nya naka base sa parent? ... paano yung sa pos client kasi pag search nila yung child out of stock?" — ang dashboard/shop/whmobile ay derived na ang display ng linked child stock (floor parent ÷ ratio) pero **ang POS client ay binabasa ang sariling StockQty ng child (laging 0)** → lumalabas "OUT" sa cashier search popup kahit may stock ang parent (sell-side guard/deduction sa SalesForm/SaleService ay parent-based na at tama — display lang ang problema). **Mga fix (client only, 3 files):** (1) `ProductService.cs` — bagong `EffectiveStockQty(p)` (child → `floor(parent.StockQty ÷ StockLinkRatio)` via local DB lookup); `GetStockStats()` → LEFT JOIN sa parent at C# compute (hindi na counted-as-OUT ang child na may stock ang parent sa TOTAL/LOW/OUT metrics); `Search(low/out)` → 2-step query (lightweight scan w/ parent join para piliin ang Ids, saka IN-fetch — walang heavy image scan sa buong catalog) para tama ang OUT OF STOCK/LOW STOCK filters. (2) `ProductSearchForm.cs` — CellFormatting ng Stock column gumagamit na ng `EffectiveStockQty` (OUT lang kung zero ang effective). (3) `ProductsForm.cs` — STOCK column cell value + kulay, detail panel txtStock (2 sites), at printed INVENTORY STOCK CHECKLIST SYS qty = effective. Build: 0 errors (11851 CA1416 warnings = pre-existing SettingsForm lang). Deployed: AppVersion → 1.1.73, exe (211,559,868 B, sha256 4a507336…b5c8e) → server drop `C:\JumongAPI\client\` (exe only — HINDI isinama ang dev JumongPos.db/logs na nasa publish output), GitHub release **v1.1.73** (id 382476733) na ginawa munang DRAFT + asset upload bago i-publish (walang 404 window) — verified `releases/latest` = v1.1.73 at download URL HTTP 200. Stores: mag-UPDATE APP.

## Latest Change (2026-09-03) — FEED POST fixes (image contain, Tingnan pa, presyo, family click, walang floating cart) — lahat verified via qwen2.5vl

**Requests sequence:** "d mabasa whole screen yung image malaki" + "text na paste ko d mabasa walang click to more" + "child nya na per box d ma click to open at mag add to cart" + "meron na cart bakit my floating cart pa?" + "pag click ng item my lalabas na card tapos my child yung per box... nag close lang yung card ng milo". **Mga fix (shop.html only, git 582b4d3 + 62a4bea):** (1) `.feed-img` → **`object-fit: contain`** (buong larawan, hindi na pinuputol ang sides). (2) **"Tingnan pa ▾/Itago ▴"** expander (`feedToggleText(id)` + `.feed-text.feed-open`) kapag text > 140 chars (dati `max-height:3.6em` clip na walang button). (3) **Product card presyo member-aware**: `fillFeedProduct` → member = `₱name · presyo`, guest = `name · 🔒 Sign in` (dati ₱0.00 dahil guest catalog walang price). (4) **Detail family/sugg click fix**: dati `onclick="closeDetail();openDetail(id)"` — ang `closeDetail`→`popOverlay` ay nag-trigger ng history pop kaya **nag-close lang ang modal**; ngayon **`openDetail(id, swap=true)`** (walang close, `pushOverlay` skip kung swap) — gumagana na ang "per box" child at "You may also like" clicks. (5) **Family cards may quick +ADD** (event.stopPropagation → addToCart ng child/box direct). (6) **Floating cart bar TINANGGAL** (`#cartBar` markup + `updateCartBar()` fn + 6 call sites sa updateBadge/closeCart/closeCheckout/closeSuccess/applyMemberUI — redundant na kasi may Cart tab). Verified: node --check · divs 208/208 · **headless phone screenshot + qwen2.5vl**: buong Milo image, "Tingnan pa" button, card "MILO CHOCO DRINK 24G (BY 12) · Sign in" (guest), walang floating cart bar (bottom nav lang) ✓. NOTE: ang MILO BOX 718 ay OUT OF STOCK sa HQ (0 pcs) kaya hindi ma-add — kailangan munang mag-receive ng stock. Git: `582b4d3`, `62a4bea`.

## Latest Change (2026-09-03) — FEEDS-ONLY cleanup: inalis ang Promo Banners / Promo Groups / FB Posts (lahat sa 📰 Feed Posts na)

**Request:** "yung promo banner wala na yan pupuntahan may feeds kana... pwede na alisin" + "burahin mo na rin, sa feeds na tayo mag adjust" — ang Feeds ang nag-iisang lugar para sa promos/updates sa home. **Inalis:** (1) Dashboard: **🎯 Promo Banners** panel + nav + `promoBannersPanel` component (kasama ang mga pickers/SAMPLE), **🎁 Promo Groups** panel + nav + `promoGroupsPanel`, **📘 FB POSTS SA HOME** card sa Shop Content + lahat ng `fbPosts` state/methods/parse/save sa `shopContentPanel` (nag-iiwan lang ng HERO/TRUST/CONTACT fields + 🏘️ subdivisions editor). (2) shop.html: buong `#legacyHome` (carousel/banners markup + FB embed section + promoSection + landingExtras) at ang kanilang JS (`renderCarousel`/`setCarousel`/`showCarousel`/`carPrev`/`carNext`/`loadBanners`/`bannerClick`/`parseFbPosts`/`fbEmbedHtml`/`fbPostUrl`/`fbCtaClick` + applyContent FB/landing sets + `promoBanners`/`promoPointsRate` vars + init calls). (3) **Link choices sa Feed composer = Product | Catalog | Wala** (walang URL — nasa loob na ng app). **Product** → item browser na may category dropdown (`browseCat`) + opsyonal na salain + **listahan agad** (hanggang 40, image+name+₱, click=piliin, ✓ chip + ✕ BAGUHIN). **Catalog** → category dropdown + **🎯 SAMPLE** (`catalogCount`/`catalogSampleNames` — "ang buong [cat] ang lalabas, N items"). Mananatili (dormant): promo overlay internal functions + `promoOrderModal` markup sa shop (hindi na na-trigger — iniingatan para sa cart logic), DB tables/endpoints (non-destructive), **⏳ Free Item Queue** panel (may old claimed items). Index cache-buster → **v=20260911**. Verified: node --check OK · divs index 695/695 · shop 210/210 · live deployed (nav/panels/FB gone, browseCat + Catalog option + buster live, shop walang legacyHome/carousel) · **headless phone screenshot + qwen2.5vl**: "top bar + purple 'Wala pang post' empty state + bottom nav — walang promo banner/floating chips/FB post/footer" ✓. Git `72fbd53`.

## Latest Change (2026-09-03) — TIKTOK-STYLE FEED sa shop home (owner posts muna) + Image-Reading Bridge (Ollama + qwen2.5vl sa dev PC)

**Request:** "tiktok nga maganda scroll scroll lang yun sana... para sa mga member pwede din sila mag post... kaso mukhang mabigat sa gb" → decision: **Phase 1 = owner/admin posts lang** (member posting = next project). Home = **full-screen vertical scroll-snap feed** (gaya ng TikTok): isang post = isang screen; right rail (❤️ like ng members + 🔗); caption sa ibaba (🏪 pangalan · 🎁 PROMO/📢 UPDATE badge · text · relative time); **product link card** (click → openDetail/order). Lahat ng images ay **auto-resize sa 1080×1920 (9:16) JPEG 0.82** sa browser bago mag-upload (hindi mabigat sa GB; walang video). Catalog/Cart/History = hindi ginagalaw. **Image-Reading Bridge**: nag-install ng **Ollama (0.33.2) sa dev PC** + `qwen2.5vl:7b` (6GB, 3080 Ti) — ang agent ay nagpo-POST ng base64 image sa `http://localhost:11434/api/generate` para basahin ang mga screenshots (ginamit sa pag-verify ng feed at sa "broken paper" diagnosis).

| File | Change |
|---|---|
| `PgDatabaseHelper.cs` | Migration: `feed_posts` (poster_type/poster_name, kind promo/update, text, image_url, link_type product/category/url, link_value, sort_order, is_active, timestamps + active idx) + `feed_likes` (post_id+customer_id PK, CASCADE). |
| `DashboardController.cs` | `GET /feed?before=&limit=` (public, likedByMe sa session; ORDER BY sort_order DESC,id DESC) · `POST /feed` (multipart; image→wwwroot/assets/feed/{guid}.ext ≤8MB; sort_order=max+1) · `PUT /feed/{id}` (image empty=keep) · `DELETE /feed/{id}` · `POST /feed/reorder` (ids→sort_order) · `POST|DELETE /feed/{id}/like` (member). GOTCHA: [FromForm]+IFormFile ay **415 sa urlencoded** — kailangan multipart (FormData). API version → 1.1.71 (2 places). |
| `components.js` | Bagong `feedPanel` (grp-ecom): composer (text · image na **canvas-resize 1080×1920 JPEG 0.82** · kind · link picker product search/category dropdown/url — reuses `_loadShopProds/_loadShopCats`) · list (thumb, badge, likes, ↑↓ reorder, EDIT, ✕) · save=multipart. |
| `index.html` | Sidebar **📰 Feed Posts** (grp-ecom) + panel section + cache-buster **v=20260910**. GOTCHA (naulit): **HUWAG gumamit ng PS Set-Content sa emoji files** — na-corrupt ang index.html (768-line diff); ni-restore sa git at re-applied via Edit tool. |
| `shop.html` | Home = feed: `#feedView.feed-snap` (scroll-snap y mandatory; **height calc(100vh−104px) + dvh @supports**; `.feed-post` ay dapat **explicit height** — lahat ng anak nito ay absolute kaya 0 ang height kung 100% lang) · `feedCard`/`loadFeed` (paged 5, sentinel IO) /`feedLike`/`feedOpenLink`/`fillFeedProduct` (product card = pangalan+₱ mula sa catalog) · `#feedPromoBar` chips (orderable promo-groups, fixed sa itaas ng bottom nav, pinupuno ng renderPromoGroups) · `#legacyHome` hidden container (carousel/FB posts/promoSection/landingExtras — iniingatan ang ids para sa JS refs) · `#homeSignIn` (guest) · catalog/cart/history = walang pagbabago. |

**Verified live:** API 1.1.71 · GET /feed [] · test post (multipart) → headless phone screenshot (390×844) + **qwen2.5vl read**: full-screen dark post "🎁 PROMO · 10 min ang nakalipas" + TEST POST MILO + promo chips ✓ → test post deleted (feed clean). PWA confirmed: sw.js network-first (API never cached) + manifest ✓. Git `2045f28` pushed. **User action:** dashboard → 📰 Feed Posts → gumawa ng posts (text + 9:16 image + link) → auto-lalabas sa home ng shop.

## Latest Change (2026-09-03) — FB POSTS multi (Shop Content: add/preview/delete) + PROMO BANNERS pickers + SAMPLE

**Request:** "yung sa ecommerce na iframe natin dapat pagka sa nakikita ko din doon sa shop content yung resulta at pwede pa ako mag add kahit ilan gusto ko at pwede ko rin idelete" + "paki damay narin yung promo banners ng katulad dyan catalog tapos target tapos sample anong item landing nya". **FB posts**: Shop Content FACEBOOK card → **`fbPosts[]` list** (key `fb_posts` = JSON array; legacy `fb_embed_home` fallback — na-migrate ang existing post, shop.html `parseFbPosts()`). Bawat post: embed code textarea + **🖼️ live preview** (`fbPreviewHtml` — extract iframe o build mula sa `data-href` kung FB script-embed ang na-paste — scripts via innerHTML ay hindi tumatakbo kaya kailangan ang extraction) + CTA pickers (Category dropdown mula sa totoong catalog + salain / Product type-to-search / URL) + **➕ ADD ANOTHER POST** + **✕ DELETE**. Home: stacked posts sa ilalim ng carousel, bawat isa may sariling CTA button (`fbCtaClick(i)`). **Promo Banners**: editor → target pickers (category dropdown + salain, product type-to-search, url) + **🎯 SAMPLE** box (pbSample — listahan ng mga item na lalabas kapag na-click ang banner: category filter + search, product name/₱, url text). Shared helpers `_loadShopCats()`/`_loadShopProds()` (cached, may category field). Cache-buster v=20260909. Web-only (walang API change). GOTCHA: kung ang FB embed code ay may `fb-root`/`.fb-post` div + script (hindi pure iframe), HINDI ito mag-re-render via innerHTML — ang fbEmbedHtml/fbPreviewHtml ay awtomatikong kumukuha ng iframe o gumagawa mula sa data-href.

## Latest Change (2026-09-03) — RESIBO: unit name sa item line (v1.1.72 client) + KOPIKO BLANCA BOX linked child (725)

**Request:** "pwede ba sa resibo iconcatenate yung item name + unit name" — ang retail receipt (`BuildReceiptLines`, PrinterService.cs:164) ay nagpi-print na ngayon ng `ProductName (UnitName)` kapag may unit (hal. `Kopiko Black Twin Pack 40g (by 10)`) — SKIP ang `pc`/`per piece` (para hindi maingay ang ordinaryong singles); ang wholesale receipt (PrintWhReceipt:1119) ay may unit na noon pa. Dahilan: isang barcode = maraming pack forms (by 5/by 10/box) — dapat malaman ng resibo kung alin ang nabenta. Client AppVersion → **1.1.72**, API latestVer → 1.1.72 (API constant stays 1.1.70). GitHub release v1.1.72 (id 381931491) + drop (211,555,772 B MZ) + API redeployed.

**KOPIKO BLANCA BOX (725):** user-created linked child `KOPIKO COFFEE BLANCA TWIN 12/10/40G` → parent 259 (Kopiko Blanca Twin), **ratio 24**, ₱1,460, walang units, barcode placeholder "wala pa blanca". Bawas: 1 box = −**24 strips** mula sa parent (qpu 1 × ratio 24 = 120 sachets = 12 by-10 sleeves) — **HINDI naapektuhan ng 2 units ng parent** (by 10 qpu2 / by 5 qpu1 — para sa parent sales lang; iisang stock pool). E-com availability = floor(parent ÷ 24). Cost note: 725 cost = ₱1,440 (hindi 24×62.5=1,500) — mag-a-auto-lock sa ₱1,500 sa susunod na save ng parent (cost lock invariant). Barcode placeholder = hindi ma-scan (kailangan ng totoong barcode).

## Latest Change (2026-09-03) — KOPIKO units re-added: by 10 DEFAULT ₱127 qpu2 + by 5 ₱65 qpu1 (2 pack forms, 1 barcode)

**Request:** "paano gagawin ko sa kopiko dalawa kasi dapat sya sa isang barcode idadaan ko sa unit? by 10 at by 5" + "by 10 ang default" — pagkatapos ng by-5 conversion, dalawang pack forms pala ang Kopiko (by-5 strip ₱65 at by-10 sleeve ₱127) sa iisang barcode. Master PUT ×3 (audited 17:52): units `by 10` (₱127, qpu **2**, is_default) + `by 5` (₱65, qpu 1) — stock base = by-5 strip (÷5 na), kaya ang by-10 ay qpu 2 (10 pcs = 2 strips), cost auto 2×base (120/125/123). **Ang USER mismo ang nag-alis ng " by 5" sa pangalan** (17:53, audit) dahil nakakalito sa unit picker (kung saan pipili kung anong by) — master = `Kopiko Black/Blanca/Brown Twin Pack 40g` (no suffix). 4 stores na-pull (verified: by 10 IsDefault=1). Resulta: 1 punch = ₱127 −2 strips (by 10 default) · by 5 = ₱65 −1 strip · isang barcode, isang stock pool. GOTCHA: huwag magulat kung mawala ang "by 5" suffix sa pangalan — ginawa iyon ng user.

## Latest Change (2026-09-03) — KOPIKO Twin Packs → by-5 conversion (MILO/BEAR template)

**Request:** "sunod naman yung kopiko blanca iconvert sa by 5 pati yung brown and black" + "oo yung twin lang" — ang 3 Twin Pack 40g products lang (257 BLACK · 259 BLANCA · 260 BROWN; HINDI kasama ang 258 "Blanca 30g by 10" at 670 "BLACK SINGLE"). Master PUT ×3 (audited 17:22): pangalan + " by 5", **₱65** lahat, cost Black **60** / Blanca **62.5** / Brown **61.5** (kasalukuyang by-5 unit values), `units:[] + clearUnits:true` (burahin pc/by 10/box). Hintayin ang POS pull (units=0 lahat ng stores). **May benta sa Blanca habang naghihintay** (HQ 362→352, AA36 240→230) — kaya nagbago ang ÷5 targets. Agent sql ÷5 + conversion trails (Synced 0 → auto-push → cloud trails 438976-438984): BLACK AA36 210→42 · E174 260→52 · A80C 250→50; BLANCA HQ 352→**70** (sobra **2 pcs** adjusted — floor) · AA36 230→46 · E174 360→72 · A80C 600→120; BROWN E174 370→74 · A80C 310→62. Cloud verified (12 rows). Resulta: 1 punch = ₱65 −1 by-5 pack; e-com ₱65/pack. Box units (₱1,460) nawala — linked child BOX kung kailangan (ratio 24: 24×65=₱1,560). GOTCHA: re-check stocks bago mag-divide — maaaring may benta habang hinihintay ang master pull (gaya ng Blanca 362→352).

## Latest Change (2026-09-03) — BEAR BRAND MILK W/ IRON 33G → by-8 pack conversion (MILO template)

**Request:** "convert natin lahat ng 41 BEAR BRAND MILK W/ IRON 33G sa by 8" — product id 41 (barcode 4800361410816, powdered milk) na-convert sa single by-8 pack gaya ng MILO: master → **₱87/cost ₱84**, pangalan na "BEAR BRAND MILK W/ IRON 33G by 8" (user na mismo ang nag-rename), **`units:[] + clearUnits:true`** (burahin ang by 8/box/pc rows — audited sa master_product_audit 17:03). Stocks divisible lahat (walang remainder): **AA36 16→2** · **E174 488→61** (HQ/ACGS 0 = skip) — agent sql UPDATE + conversion StockTrail (Synced 0 → auto-push → cloud trails 438880/438881, verified cloud stock 2/61). Resulta: 1 punch = ₱87 −1 pack; e-com reserve = 1 pack. GOTCHA: hintayin munang ma-pull ang master (units=0 sa lahat ng 4 stores) BAGO mag-divide — gaya ng MILO. Box unit nawala — kung kailangan ang box, gagawa ng linked child sa dashboard (ratio 24, 24×₱87=₱2,088) gaya ng MILO BOX 718.
...
### Same-day follow-up — RETRO REPAIR (pang-kalahatan): 22 sales / 16 QR customers / 64 pts (+ per-receipt history)

**User:** "nataon sya lang nag reklamo pero kung gagawa ka ng kaayusan dba dapat pang kalahatan yan?" — at itinama: "paano na record sa kanya kung d na ka link" — ang LAHAT ng 4 na resibo ni Retchie ay **Counter** (customer attached, customer_id 1638) pero 3 ang 0 points. **TOTOONG root cause (hindi duplicates):** ang per-item `PointsEarned` sa `SaveSale` (SaleService.cs:82-97) ay nakadepende sa **PointsExempt FLAG ng produkto SA ORAS NG BENTA** — noong Aug 27-31 ang snacks/groceries ay naka-exempt pa sa master (kaya 0 ang 0142/0123/0028 kahit may ₱700+ snacks), habang ang Coke 1.5/CLVB noon ay HINDI exempt (kaya kumita ang 0107 ng 5); ngayon baliktad ang flags (yosi/alcohol/soda exempt, snacks earn). Na-verify: HQ local "Retchie De Guzman" = isang malinis na row na may QR (AS-879244, id 1146) — walang duplicates; AA36/E174/A80C = zero duplicate-name customers (agent GROUP BY HAVING COUNT>1 = walang laman).

**Ang ginawa (server-side only, walang client release):**
1. **Retro-credit sale_items** — DO block: per non-voided QR-customer sale, `expected = floor(Σ current-flags items)` vs `stored = Σ points_earned`; kung expected > stored → idagdag ang delta sa **huling non-exempt item** (parehong top-up pattern ng POS) → **UPDATE 22 sales** (HQ 17 + AA36 3 + ...), kasama ang Retchie 0142 +3, 0123 +1, 0028 +1 (0107 +5 hindi ginalaw — expected < stored, NEVER decrease).
2. **Balance repair re-run** (GREATEST) → **UPDATE 15 accounts** (Retchie 5→**10**, KrisBee 9→**12**, Marischel 5→**7**; Ana 35/Zeny 25 hindi nagalaw).
3. **ORDER HISTORY** (DashboardController.cs:355) = `SUM(sale_items.points_earned)` → **may earn na ang bawat resibo sa history** (+5/+3/+1/+1 kay Retchie) ✓.

**Caveat (documented):** ang retrofit ay CLOUD-ONLY — ang local `sale_items` ng stores ay nananatiling 0 para sa mga lumang resibo; kung ma-void balang-araw ang isang retro-credited receipt, ang reversal ay 0 (local stored) habang may retro sa cloud → maliit na over-credit lang, tinatanggap. GOTCHA: ang psql DO block ay nangangailangan ng single-quoted here-string sa PowerShell (`$$` ay in-interpolate ng PS bilang automatic variable → "syntax error at or near DECLARE").
...
| File | Change |
|---|---|
| `Services/PrinterService.cs` | Reconciliation formula: `expected = previousInventory + totalStockReceivedCost + voidReturns + saleTrailsCost - adjustDown` → **`+ adjustDown`** (adjustDown ay negative — identity: Expected = Prev + Received + VoidReturns + SaleTrails(neg) + AdjustDown(neg)). Display guards: `adjDownTransfers > 0`/`adjDownEcom > 0`/`adjDownMobile > 0`/`adjustDown > 0` → **`!= 0`** + `Math.Abs(...)` (negative values kaya hindi kailanman nag-fire ang `> 0` → breakdown lines never printed); `adjManual = Math.Abs(adjustDown) - Math.Abs(...)`. |
| `Services/EmailService.cs` | Pareho: Expected row + Variance row (4 expressions) `- adjustDown` → `+ adjustDown`; guards `> 0` → `!= 0` + Math.Abs sa lahat ng adj lines. |
| `Services/SyncService.cs` | `GetEndShiftSnapshot` URL: `/dashboard/dashboard/end-shift-snapshot` → **`/dashboard/end-shift-snapshot`** (double prefix = 404 → null → channel breakdown never fetched). |
| `Forms/EndShiftForm.cs` | History reprint: `prevInvReprint = dc.TotalInventoryCostPrev > 0 ? dc.TotalInventoryCostPrev : DailyCloseService.GetPreviousInventoryCost(dc.Id)` — pre-1.1.68 closes (TotalInventoryCostPrev column = 0) ay tama pa rin ang Previous Inventory. |
| `Services/AppVersion.cs` | `"1.1.68"` → `"1.1.69"`. |
| `JumongCloudAPI/Controllers/DashboardController.cs` | `GetEndShiftSnapshot` ecommerce query: `WHERE status IN ('confirmed','shipped','delivered') AND created_at >= @since` → **`WHERE status = 'delivered' AND delivered_at >= @since`** (count by shift activity — orders na-deliver during the shift, hindi placement time; tugma sa `order_payments` COD collections). `latestVer` → `"1.1.69"` (API version constant stays 1.1.62). |

**Deployed:** client v1.1.69 (exe 211,506,620 B, sha256 5f4b5c40…, MZ verified) → drop `C:\JumongAPI\client\`; **GitHub release v1.1.69** (id 379931155, asset 538303432, raw `--data-binary`, state=uploaded, size exact). API redeployed (WinRM stop→copy→start, version 1.1.62 + ecommerce query live verified). **LAHAT NG 4 STORES NA-UPDATE via agents** (HQ `JumongPosHW\`, ACGS `C:\JumongPos\`, HVR `HVR_POS\`, U Got Mart `C:\JumongPos\`): exe download `https://admin.jumongdev.com/tmp/JumongPosV1.01.exe` → each "Downloaded 211506620 bytes" ✓ + restart; agents all alive/heartbeatting. GOTCHA (agent results): ang `/agent/results/{store}` ay return-then-clear at ang dashboard AGENTS tab ay kumokonsumo ng buffer — poll kaagad pagkatapos ng ~5-10s; ang `ps` checks na may complex quoting ay minsan hindi nag-publish ng result (stale entries lang ang nakikita). appVersion sa agent/status ay nag-a-update lang pagkatapos ng next cashier login (Program.cs writes post-login) — ang exe replacement ang source of truth. POST-VERIFY: HQ end-shift → Expected = Actual ≈ BALANCED + CHANNEL BREAKDOWN shows `E-commerce: N order(s)` + COD collected. Git commits pending (v1.1.68-era working tree + v1.1.69). Dev PC shutdown requested after rollout.

## Previous Change (2026-08-31) — DriverApp v2.0.4: DATE + TIME sa deliveries list + order detail (buong petsa, may taon)
...
| File | Change |
|---|---|
| `JumongCloudAPI/Data/PgDatabaseHelper.cs` | Migration: `ALTER TABLE sales ADD COLUMN IF NOT EXISTS customer_name TEXT` + **`customer_updates`** table (id, customer_id, customer_name, message, read_at, created_by, created_at + customer idx). |
| `JumongCloudAPI/Controllers/SyncController.cs` | **SyncSales remap**: ang POS ay nagpapadala ng LOCAL customer id — ngayon ay nireresolve ang **cloud customer id via the UNIQUE customer name** (kung may match) at ini-store ang `customer_name` — lahat ng future POS sales sa kahit anong store ay naka-link na sa tamang cloud customer. |
| `JumongCloudAPI/Controllers/DashboardController.cs` | **`GET /customer/transactions`** (member): online orders + POS sales (customer_id=cloud id O customer_name) + wholesale (customer_name) — may type/storeName/date, sorted desc. **`GET /customer/receipt?storeId=&invoiceNo=`** (ownership-gated): sale + items (sale_items naka-link sa **pos_id**, hindi cloud id!). **`GET /customer/updates`** + **`POST /customer/updates/read`**. Admin: **`POST/GET /customers/{id}/updates`**. **`CustomerOrdersById`** (dashboard modal) → combined: ecommerce + POS + wholesale na may `type` + storeName + **points (SUM sale_items.points_earned)**. **`UpdateCustomerPhone`** (PUT /customers/{id}/phone, dup-checked). **`GetCustomers`** → walang store filter (customers universal — ang store-filtered view ay 0 palagi; ito ang dahilan kung bakit hindi nakakakita ng customers ang non-admin accounts tulad ng abi) + bagong fields: isSariSari, googleSub, default address (LATERAL customer_addresses). |
| `Services/SyncService.cs` | **SyncSale payload + `CustomerName`** (v1.1.62). **`GetHeldStockAsync`** (v1.1.60). |
| `Forms/SalesForm.cs` + `Services/SaleService.cs` | **v1.1.61 LOYALTY POINTS FIX**: per-item truncation → decimal accumulation + floor ONCE (₱100+₱100 = 1 point, gaya ng online orders); SaveSale stores per-item floors + **top-up ng remainder sa huling non-exempt item** (SUM(PointsEarned) = awarded total para tama ang void reversal). **`HeldNote`** — "N pcs naka hold sa online order" sa lahat ng guards. |
| `JumongCloudAPI/wwwroot/shop.html` | **🔔 bell sa header** (red unread badge) → updates modal (BAGO highlight, auto mark-read). **🕐 History = ALL transactions** (loadHistory: online rich cards + in-store/wholesale cards na may 🏪/📦 badges + 🧾 RESIBO button → receipt modal). **📝 profile banner** sa sticky header kapag may Gmail pero walang phone/address. Promo overlay (typeable qty + FREE row), catalog sort (category A-Z), category selector A-Z, mobile category rail spacing, stock control sa cart. |
| `JumongCloudAPI/wwwroot/index.html` + `components.js` | Customers panel: **Status badge** (🏪 STORE / REGULAR), **Address column** (Blk/Lot/Subd/details), **📱 I-SAVE PHONE** button+modal (Google customers na walang phone — 20 ang naka-flag), **📢 UPDATE** button+modal (thread ng customer_updates), 📱 NO PHONE filter pill; ORDER HISTORY modal → LAHAT ng transactions (Type badges). |
| `JumongCloudAPI/Data/PgDatabaseHelper.cs` | **Disk-full crash fix**: ang startup migration ay nag-DROP+ADD ng UNIQUE constraints sa BAWAT boot (sale_items 80k+ rows rebuild = temp files) — nung puno ang disk (System Restore/VSS 4GB) → "No space left on device" → crash-loop ~21min (520/ERR_CONNECTION_CLOSED). Ngayon: **guarded ADD (IF NOT EXISTS pg_constraint)** + try/catch sa block. |
| Versions | Client **1.1.62** (211,436,988 B, GitHub release 377606439); API latestVer 1.1.62; API version constant 1.1.60. |

**Verified live:** KrisBee Hong (cloud 1668 = QR AS-8328AE; sale 0039 na-manual-fix ang customer_id 1151→1668): transactions → `INV-7159-20260827-0039 · Andengs Superstore - HQ · ₱1,805.50` ✓; receipt → **21 items** ✓; points sa modal → `5 pts → +5` ✓; updates post → unread 1 → read → 0 ✓; `customer_sessions` test tokens para sa CDP/curl tests (customer 1454/1668). Retro-credit KrisBee: **+5 points** (local 1151 + cloud 1668 + sale_items sum=5) para sa 0039 (₱1,125.50 non-exempt ÷ 200 = 5.62 → 5). Nag-post ng update sa account ni KrisBee ("Naayos na po ang inyong points! +5..."). GOTCHAS: sale_items naka-link sa **sales.pos_id** (hindi cloud id); ang sale sync ay nagpapadala ng **local** customer id — nireresolve na sa server via unique name; ang `customers` ay universal (store_id='') kaya WALANG store filter sa customer queries; ang dashboard cache-bust ay `?v=20260827`. Git: 0dd19a7 (promo overlay), 5366b9b, 1fdfbbe, ecfa4a4, 95ed7f1 (sort), 2d04883 (cat A-Z), c8ecd74, 390d4a3, 82a6945 (banner), 7d9b809 (hold), 9d8a5ae (points+abi), 5c68de0 (units wipe), cbf3181 (customers panel), 449b613 (AGENTS reorg), 77ab726 (v1.1.62), d018411, 439e1dd, 227c007 (disk fix). HQ sa v1.1.62 via agent (211,436,988 B) ✓; HVR/ACGS/Naic via UPDATE APP (need nila ang v1.1.62 para mag-link ang kanilang POS sales sa cloud customers).

## Previous Change (2026-08-27) — v1.1.60 (API + POS client) + web: E-COMMERCE STOCK HOLD — items HELD at SUBMIT (not CONFIRM) + HQ POS "naka hold sa online" message + cancel releases all

**Request:** "pag na submit ng ecomerce customer ang order nila naka hold na sa inventory mga items binili para sa local HQ pos... kung 1 na lang eh na order sa online sabihin 1 nalang naka hold sa online pero kung marami pa naman stock tuloy lang ang benta basta yung order online naka hold item pero pag cancel order balik lahat yung naka hold sa inventory" — previously the stock reservation happened at **CONFIRM** (pending orders didn't protect stock); now **SUBMIT reserves immediately** and the HQ POS tells the cashier why an item can't be sold.

| File | Change |
|---|---|
| `JumongCloudAPI/Data/PgDatabaseHelper.cs` | Migration: `ALTER TABLE online_orders ADD COLUMN IF NOT EXISTS stock_held BOOLEAN NOT NULL DEFAULT FALSE`. |
| `JumongCloudAPI/Controllers/DashboardController.cs` | **`ShopCreateOrder`**: after items + promo free line, **`AdjustOrderStockHeld(conn, tx, orderId, orderNo, customerName, reserve)`** (NEW shared helper — reads the order's CURRENT `online_order_items`, resolves barcode + qpu + HQ `products` row, guarded deduct (reserve) / restore (release), writes server-side trails `pos_id=-NEXTVAL`, ref `SHOP-<no> HOLD/RELEASED -> <customer>`, channel ecommerce) — reserve at submit → **shortage = 400 "Insufficient HQ stock" + ROLLBACK (order NOT created)** + `stock_held=true`. **`ShopUpdateOrderStatus`**: reads `stock_held`; CONFIRM reserves only `!stockHeld` (legacy pre-change orders); CANCEL restores when `stockHeld || curStatus != 'pending'`; main UPDATE sets `stock_held = CASE WHEN cancelled THEN FALSE WHEN confirmed THEN TRUE ELSE stock_held`. **`ShopUpdateOrderItems`** (edit): pending → `AdjustOrderStockHeld(..., false)` release before rewrite + `AdjustOrderStockHeld(..., true)` re-hold after (shortage → 400 rollback); confirmed → existing delta loop unchanged. **`DriverCancelOrder`**: UPDATE adds `stock_held = FALSE` (restore loop already released). **NEW `GET /warehouse/stock-held?ids=`** — per-pos held pcs: `SUM(oii.qty × COALESCE(u.qty_per_unit,1))` from active orders (`pending/confirmed/shipped/arrived`) joined via master barcode → `[{productId(=pos_id), heldPcs}]` (cap 50 ids). latestVer → `"1.1.60"`. |
| `Services/SyncService.cs` | **NEW `GetHeldStockAsync(IEnumerable<int>)`** — HQ-gated mirror of GetLiveStockAsync → `/dashboard/warehouse/stock-held?ids=` → dict posId→heldPcs (empty on fail/non-HQ). |
| `Forms/SalesForm.cs` | **NEW `HeldNote(int productId)`** — HQ-gated: if held>0 returns `" — N pcs naka hold sa online order"`; appended to ALL 4 guard messages: add-to-cart (`AddToCart`), qty-change dialog, pre-pay live check (`liveIssues`), held-cart retrieve warning. |
| `Services/AppVersion.cs` | `"1.1.59"` → `"1.1.60"`. |
| `JumongCloudAPI/wwwroot/shop.html` | (earlier same session, separate commits) **PROMO ORDER OVERLAY** (Home 🎁 card → modal: group items with **typeable qty inputs** (`promoQtySet` clamps sa stock) + ± steppers + total pcs counter + "Kwalipikado!" note) → **ADD TO CART** adds items + **FREE ₱0 row** (`promoLineHtml`: 🎁 FREE name · ₱0.00 × N · [FREE] badge) kapag eligible; **stock display per item**; **FULL-WIDTH promo cards** (`w-full` vertical stack, no scroll) + trust-badges `-mt-8` overlap REMOVED; **CART STOCK CONTROL**: cart qty is now a **typeable input** (`setQtyTyped` + `renderCartTotals()` — no focus loss) + `setQty` **unit-aware** (row's `unitName`, not defUnit) + blocks when `avail <= 0` ("Out of stock na") + clamps with "Only N available — naka-adjust". |

**Verified live (API 1.1.60, test order id 23 cleaned):** create order (Coke Mismo 002, 5 pc) → stock 107→102 + trail `-5 | SHOP-20260827-0023 HOLD -> CDP Hold Test` + `stock_held=t` + `/warehouse/stock-held?ids=5525` → `[{5525,5}]`; CANCEL → stock back 107 + trail `+5` + `stock_held=f` + held=[]; shortage qty 99999 → 400 "Insufficient HQ stock: pc x99999 (002)" + NO order row + stock unchanged. **GOTCHA (PS quoting):** PowerShell 5.1 native-arg passing mangles inline JSON for curl (`'{"status":...}'` sent with literal quotes → ASP.NET "invalid start of a property name") — ALWAYS write JSON to a temp file + `--data-binary "@file"`. Release v1.1.60 (client 211,432,892 B) upload via raw `--data-binary`; **stores update via UPDATE APP — pagkatapos i-update, ang HQ POS ay magsasabi "N pcs naka hold sa online order" kapag na-block ng online hold (may API 1.1.60 na ang server)**. Git commits: `0dd19a7` (promo overlay), `5366b9b` (stock display), `1fdfbbe` (typeable qty), `ecfa4a4` (full-row cards + overlap fix), + this feature commit (pending).

## Previous Change (2026-08-25) — v1.1.59 (API) + web: MODERN MOBILE SHOP — AI chat REMOVED, Home/Catalog split, bottom nav, RESTOCK REQUESTS + PRODUCT SUGGESTIONS
...
**Delivery VERIFIED END-TO-END (2026-08-26):** webhook log shows real `POST entry messaging=1` (22:19:45, tester Andrea De PSID 27205704152410631) → `ProcessMessengerMessage` ran → AI reply saved to `messenger_convos` + sent back. Bot now: receives ✓ replies ✓ sends link+QR ✓ saves history ✓. Also proven SEND via `POST /me/messages` (message_ids returned for Andrea + Caim Grocer PSIDs from `/conversations`). Dev-mode tester acceptance (App Roles → Testers, confirm URL `https://developers.facebook.com/apps/1041403252013127/roles/testers/confirm`) was the final gate for webhook delivery; roles API shows only admins (pending testers don't appear via API). **CATALOG CAROUSEL added** (user: "ang chatgenie may ecommerce ang messenger tayo ba?"): keywords `catalog/products/browse/listahan/produkto` → `SendCatalogCarousel` (Generic Template, up to 10 in-stock sell_online products w/ images, subtitle price+category, ORDER NOW web_url → shop.jumongdev.com + quick replies order/delivery/promo); new public `GET /product-image/{id}` serves master_products.image_data as image/jpeg (Cache-Control 1h) — the template image_url. Verified: `catalog carousel send=OK elements=10`. **POINTS + ORDER STATUS via PSID linking** (user: "paano ma ilink ang customer... kunwari ask ng points nila"): new `customers.psid` column (+idx); Messenger keywords `points/puntos/loyalty` → points lookup; `order ko/nasaan order/track/status ng order/kamusta order` → last-3 orders summary (orderNo/status/₱/date); linking = phone (09xx/63xx normalized) OR **Customer ID AS-XXXXXX (qr_code)** OR numeric id — `_msgrPending` state machine asks for the identifier once, then saves PSID on the customer (UPDATE customers SET psid) so the next query is instant (no prompt). Answers: ⭐ points + QR-eligibility note; 📦 order list or "wala pang order". Tested via webhook (PSID 27205704152410631 → "points" → prompt → "AS-AE9ED3" → linked to customer 1481 + points reply; cleanup: psid cleared). **PROFILE FOLLOW-UP (fill-phone)**: kapag linked customer ay walang mobile number sa profile → sinasabi ng bot "Wala po kayong MOBILE NUMBER na naka-save... ako na po mismo ang maglalagay" → kapag nag-type sila ng phone → `_msgrPending = "fill-phone:<cid>"` → `FillPhone` (ExtractPhone, duplicate-phone check, UPDATE customers SET phone — **NOTE: customers table has NO updated_at column** — na-fix). Verified live (scratch customer 1654: points → prompt → 09171234567 saved, cleaned). **PRIVACY + APP REVIEW PREP**: `privacy.html` live (admin.jumongdev.com/privacy.html, DPA-compliant) + `Desktop/app_review_submission.txt` (use case description, steps: Business Verification → App Review pages_messaging → Live mode). User decision: kailangan muna ng business permit bago mag-App Review. API redeployed (1.1.59). Git commit pending.
...
## Previous Change (2026-08-13) — v1.1.22 (Cloud API) + web-only: Per-Resibo Credit Payment (Mobile)
...
## Previous Change (2026-08-13) — v1.1.21 (Cloud API): E-Commerce Shop Backend
...
## Complete Change History - v1.0.x (POS client + Cloud API, pinakaluma -> pinakabago)
...
## Complete Change History - v1.1.x + infra/web notes (2026-08 onward, pinakaluma -> pinakabago)
