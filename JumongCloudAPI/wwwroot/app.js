...
window.exportCSV = (name) => {
  const map = {
    ...
    shifts: ['Close Date,Store,Sales,Cash,E-Wallet,Credit,Voided,Expenses,Cash on Hand,Variance,Inv Cost,COGS,Recv Cost,User,Checked By,Checked At', d => d.map(x => [x.closeDate, x.storeId, x.totalSales, x.totalCash, x.totalEwallet, x.totalCredit, x.totalVoided, x.totalExpenses, x.cashOnHand, x.difference, x.totalInventoryCost, x.totalCostSold, x.totalStockReceivedCost, x.userName, x.checkedBy || '', x.checkedAt || ''])],
    invcost: ['Date,Store,Inv Cost,COGS,Recv Cost,Expected,Variance', d => d.map(x => [x.closeDate, x.storeId, x.totalInventoryCost, x.totalCostSold, x.totalStockReceivedCost, (x.totalInventoryCost||0)+(x.totalStockReceivedCost||0)-(x.totalCostSold||0), (x.totalInventoryCost||0)-((x.totalInventoryCost||0)+(x.totalStockReceivedCost||0)-(x.totalCostSold||0))])],
  ...
  if (name === 'st-transfer') {
    ...
    const headers = 'ID,Client,From,Source,Status,Notes,Date,Created By,Via,Accepted';
    const rows = st.transfers.map(x => [x.id, x.clientName, x.fromStoreId || (x.source === 'hq' ? 'HQ' : (x.source === 'warehouse' ? 'WH' : '')), x.source || '', x.status, x.notes, x.createdAt, x.createdBy, x.createdVia, x.receivedAt]);
